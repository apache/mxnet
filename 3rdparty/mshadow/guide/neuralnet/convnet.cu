...
class ConvNet : public INNet {
 public:
  ...
  ConvNet(int batch_size, int insize, int nchannel, int ksize, int kstride, int psize, int num_out)
    ...
    Ki2h.set_stream(stream);
    g_Ki2h.set_stream(stream);
    ...
    nhidden.Resize(Shape4(
        batch_size, nchannel, (insize - ksize) / kstride + 1, (insize - ksize) / kstride + 1));
    nhiddenbak.Resize(nhidden.shape_);
    npool.Resize(Shape4(batch_size,
                        nchannel,
                        (nhidden.size(2) + 1 - psize) / psize,
                        (nhidden.size(3) + 1 - psize) / psize));
    ...
    obias = 0.0f;
    // setup weights
    Ki2h.Resize(Shape2(nchannel, ksize * ksize));
    g_Ki2h.Resize(Ki2h.shape_);
    Wh2o.Resize(Shape2(nflat.size(1), num_out));
 ...
 private:
  ...
  inline static void ConvForward(const Tensor<xpu, 4, real_t>& in,
    ...
    out = swapaxisexp<1, 0>(reshape(tmp_dst, Shape4(nchannel, nbatch, oheight, owidth)));
  }
  ...
  inline static void ConvBackWard(const Tensor<xpu, 4, real_t>& out,
    ...
    tmp_col  = unpack_patch2col(in, ksize, ksize, kstride, kstride, 1, 1);
    tmp_dst  = reshape(swapaxisexp<1, 0>(out), tmp_dst.shape_);
