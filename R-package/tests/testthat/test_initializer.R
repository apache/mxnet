...
testthat("mx.init.Xavier", {
  ...
  for(dist_type in c('gaussian', 'uniform')) {
    for(factor_type in c('in', 'out', 'avg')) {
      xavier_init <- mx.init.Xavier(rnd_type = dist_type, factor_type = factor_type, magnitude = 200)
      expect_equal(typeof(xavier_init), 'closure')
      ...
      if(dist_type == 'gaussian') {
        ...
        weight_sd = sd(as.array(X_weight))
        expect_equal(weight_mean, 0, tolerance=1e-2)
        expect_equal(weight_sd, scale, tolerance=1e-2)
      } else {
        expect_equal(X_weight >= -scale, mx.nd.ones(shape))
        expect_equal(X_weight <= scale, mx.nd.ones(shape))
        weight_mean <- mean(as.array(X_weight))
...
testthat("mx.init.internal.default", {
  ...
  sample_beta <- mxnet:::mx.init.internal.default("X_beta", c(5, 10, 100),
                                          ctx = mx.ctx.default())
  expect_equal(sample_beta, mx.nd.zeros(c(5, 10, 100)))

  sample_moving_mean <- mxnet:::mx.init.internal.default("X_moving_mean",
                                                 c(5, 10, 100),
                                                 ctx = mx.ctx.default())
  expect_equal(sample_moving_mean, mx.nd.zeros(c(5, 10, 100)))

  sample_moving_var <- mxnet:::mx.init.internal.default("X_moving_var",
                                                c(5, 10, 100),
                                                ctx = mx.ctx.default())
  expect_equal(sample_moving_var, mx.nd.ones(c(5, 10, 100)))

  expect_error(mxnet:::mx.init.internal.default("X", c(5, 10, 100),
                                                ctx = mx.ctx.default()),
