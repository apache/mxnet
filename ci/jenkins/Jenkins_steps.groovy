    ...
    return ['GPU: CMake MKLDNN': {
      node(NODE_LINUX_CPU) {
        ws('workspace/build-cmake-mkldnn-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.init_git()
            utils.docker_run('ubuntu_gpu_cu111', 'build_ubuntu_gpu_cmake_mkldnn', false)
        ...
        ws('workspace/ut-python3-quantize-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            try {
              ...
              utils.docker_run('ubuntu_gpu_cu111', 'unittest_ubuntu_python3_quantization_gpu', true)
              utils.publish_test_coverage()
            } finally {
    ...
    return ['Python Integration GPU': {
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/it-python-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.unpack_and_init('gpu', mx_lib)
            utils.docker_run('ubuntu_gpu_cu111', 'integrationtest_ubuntu_gpu_python', true)
            utils.publish_test_coverage()
          }
        }
      }
    ...
    return ['Caffe GPU': {
        node(NODE_LINUX_GPU) {
            ...
            timeout(time: max_time, unit: 'MINUTES') {
                ...
                utils.docker_run('ubuntu_gpu_cu111', 'integrationtest_ubuntu_gpu_caffe', true)
                utils.publish_test_coverage()
            }
            }
        }
      ...
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/it-capi-cpp-package') {
          timeout(time: max_time, unit: 'MINUTES') {
            ...
            utils.docker_run('ubuntu_gpu_cu111', 'integrationtest_ubuntu_gpu_capi_cpp_package', true)
            utils.publish_test_coverage()
          }
        }
      }
    ...
    return ['Scala: GPU Makefile': {
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/ut-scala-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.unpack_and_init('gpu', mx_lib)
            utils.docker_run('ubuntu_gpu_cu111', 'integrationtest_ubuntu_gpu_scala', true)
            utils.publish_test_coverage()
          }
        }
      }
      ...
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/ut-cpp-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            ...
            utils.docker_run('ubuntu_gpu_cu111', 'unittest_cpp', true)
            utils.publish_test_coverage()
          }
        }
      }
    ...
    return ['Perl: GPU Makefile': {
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/ut-perl-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.unpack_and_init('gpu', mx_lib)
            utils.docker_run('ubuntu_gpu_cu111', 'unittest_ubuntu_cpugpu_perl', true)
            utils.publish_test_coverage()
          }
        }
      }
    ...
    return ['R: GPU': {
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/ut-r-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.unpack_and_init('gpu', mx_lib)
            utils.docker_run('ubuntu_gpu_cu111', 'unittest_ubuntu_gpu_R', true)
            utils.publish_test_coverage()
          }
        }
      }
