    ...
    return ['GPU: oneDNN': {
      node(NODE_LINUX_CPU) {
        ws('workspace/build-onednn-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.init_git()
            utils.docker_run('ubuntu_gpu_cu114', 'build_ubuntu_gpu_onednn', false)
    ...
    return ['GPU: oneDNN-CUDNNOFF': {
       node(NODE_LINUX_CPU) {
         ws('workspace/build-onednn-gpu-nocudnn') {
           timeout(time: max_time, unit: 'MINUTES') {
             utils.init_git()
             utils.docker_run('ubuntu_gpu_cu114', 'build_ubuntu_gpu_onednn_nocudnn', false)
             utils.pack_lib(lib_name, mx_onednn_lib)
           }
         }
       }
    ...
    return ['GPU: CUDA+cuDNN': {
      node(NODE_LINUX_CPU) {
        ws('workspace/build-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.init_git()
            utils.docker_run('ubuntu_gpu_cu114', 'build_ubuntu_gpu', false)
    ...
    return ['GPU: CUDA+cuDNN, debug': {
      node(NODE_LINUX_CPU) {
        ws('workspace/build-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.init_git()
            utils.docker_run('ubuntu_gpu_cu114', 'build_ubuntu_gpu_debug', false)
            utils.pack_lib(lib_name, mx_lib_cpp_examples)
          }
        }
      }
    ...
    return ['GPU: Clang10 -WError': {
      node(NODE_LINUX_CPU) {
        ws('workspace/build-cpu-clang10') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.init_git()
            utils.docker_run('ubuntu_gpu_cu114', 'build_ubuntu_gpu_clang10_werror', false)
            utils.pack_lib(lib_name, mx_lib)
          }
        }
      }
      ...
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/ut-python3-gpu') {
          try {
            ...
            python3_gpu_ut_cython('ubuntu_gpu_cu114')
            utils.publish_test_coverage()
          } finally {
      ...
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/ut-python3-onednn-gpu') {
          try {
            ...
            python3_gpu_ut('ubuntu_gpu_cu114')
            utils.publish_test_coverage()
          } finally {
    ...
    return ['Python3: oneDNN-GPU-NOCUDNN': {
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/ut-python3-onednn-gpu-nocudnn') {
          try {
            utils.unpack_and_init(lib_name, mx_onednn_lib)
            python3_gpu_ut_nocudnn('ubuntu_gpu_cu114')
            utils.publish_test_coverage()
          } finally {
    ...
    return ['byteps tests GPU': {
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/it-byteps') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.unpack_and_init(lib_name, mx_lib)
            utils.docker_run('ubuntu_gpu_cu114', 'integrationtest_ubuntu_gpu_byteps', true, '32768m')
            utils.publish_test_coverage()
          }
        }
      }
    ...
    return ['dist-kvstore tests GPU': {
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/it-dist-kvstore') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.unpack_and_init(lib_name, mx_lib)
            utils.docker_run('ubuntu_gpu_cu114', 'integrationtest_ubuntu_gpu_dist_kvstore', true)
            utils.publish_test_coverage()
          }
        }
      }
      ...
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/it-cpp-package-gpu') {
          timeout(time: max_time, unit: 'MINUTES') {
            ...
            utils.docker_run('ubuntu_gpu_cu114', 'integrationtest_ubuntu_cpp_package_gpu', true)
            utils.publish_test_coverage()
          }
        }
      }
    ...
    return ['Data Interchange': {
      node(NODE_LINUX_GPU_G4) {
        ws('workspace/it-data-interchange') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.unpack_and_init(lib_name, mx_lib)
            utils.docker_run('ubuntu_gpu_cu114', 'test_python3_data_interchange_gpu', true)
            utils.publish_test_coverage()
          }
        }
      }
    ...
    return ['Python Docs': {
      node(NODE_LINUX_GPU) {
        ws('workspace/docs') {
          timeout(time: max_time, unit: 'MINUTES') {
            utils.unpack_and_init(lib_name, mx_lib_cython)
            utils.docker_run('ubuntu_gpu_cu114', 'build_python_docs', true)
