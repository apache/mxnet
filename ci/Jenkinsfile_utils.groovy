...
def docker_run(platform, function_name, use_nvidia = false, shared_mem = '500m', env_vars = "",
               build_args = "") {
  def command = "ci/build.py %ENV_VARS% %BUILD_ARGS% --docker-registry ${env.DOCKER_ECR_REGISTRY}_${env.CHANGE_TARGET} %USE_NVIDIA% --platform %PLATFORM% --docker-build-retries 3 --shm-size %SHARED_MEM% /work/runtime_functions.sh %FUNCTION_NAME%"
