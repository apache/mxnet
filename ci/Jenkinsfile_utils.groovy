...
def docker_run(platform, function_name, use_nvidia, shared_mem = '500m', env_vars = "", retries = '3') {
  def command = "ci/build.py %ENV_VARS% --docker-registry ${env.DOCKER_CACHE_REGISTRY} %USE_NVIDIA% --platform %PLATFORM% --docker-build-retries %RETRIES% --shm-size %SHARED_MEM% /work/runtime_functions.sh %FUNCTION_NAME%"
  ...
  command = command.replaceAll('%RETRIES%', retries)
