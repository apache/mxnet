...
def write_libmxnet_meta(args: argparse.Namespace, destination: str):
    ...
    the libmxnet library information (commit id, branch, type, etc.).
...
def get_branch_from_cmd() -> Optional[str]:
    """
    Returns the output of 'git rev-parse --abbrev-ref HEAD'
    :return: A commit id, or None if the command fails
    """
    try:
        logger.debug('Executing "git rev-parse --abbrev-ref HEAD"')
        branch = check_output("git rev-parse --abbrev-ref HEAD".split(" ")).decode('UTF-8').strip()
        logger.debug('Found branch: {}'.format(branch))
        return branch
    except CalledProcessError as e:
        logger.debug('Error determining git branch')
        logger.debug(e)
        return None
...
def probe_branch() -> str:
    """
    Probes the system in an attempt to determine the mxnet git branch
    :return: The git branch of the mxnet repo, or None if not found
    """
    logger.debug('Probing for git branch')
    branch = os.environ.get('MXNET_BRANCH')
    if not branch:
        logger.debug('MXNET_BRANCH environment variable not set. Trying GIT_BRANCH')
        branch = os.environ.get('GIT_BRANCH')
    if not branch:
        logger.debug('GIT_BRANCH environment variable not set. Trying git command')
        branch = get_branch_from_cmd()
    if not branch:
        logger.debug('Could not determine git branch')
    else:
        logger.debug('Branch is: {}'.format(branch))
    return branch
...
def probe(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not args.branch:
        branch = probe_branch()
        if not branch:
            logger.error('Could not determine git branch. '
                         'Please set branch manually with --branch or ensure you are in a cloned '
                         'mxnet repository directory')
            sys.exit(1)
        args.branch = branch
...
def get_s3_key_prefix(args: argparse.Namespace, subdir: str = '') -> str:
    ...
    """
    prefix = "{branch}/{git_sha}/{lib_type}/{os}/{variant}/".format(**vars(args))
    if subdir:
...
def push_artifact(args: argparse.Namespace):
    ...
    logger.info('BRANCH      : {}'.format(args.branch))
