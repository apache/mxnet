...
# Calculates the docker image tags given the mxnet variant
# and whether release build or not
...
# If this is a release version, extract mxnet version from libinfo.py
if [[ ${is_release} == "true" ]]; then
    version=`grep -Ei '__version__ = "[0-9].[0-9].[0-9]"' python/mxnet/libinfo.py | grep -oEi '[0-9].[0-9].[0-9]'`
fi
