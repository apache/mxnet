...
class TestArtifactRepositoryTool(unittest.TestCase):
    ...
    @patch('artifact_repository.check_output')
    def test_get_branch_from_cmd_returns_none_on_fail(self, mock):
        """
        Tests get_branch_from_cmd returns None if the command fails
        """
        not_sucessful = 255
        mock.side_effect = CalledProcessError(cmd="some command", returncode=not_sucessful)
        self.assertIsNone(get_branch_from_cmd())
    ...
    def test_probe_branch_mxnet_branch(self):
        """
        Tests the value of MXNET_BRANCH env var is returned
        """
        with patch.dict('os.environ', {'MXNET_BRANCH': 'master'}):
            self.assertEqual(probe_branch(), 'master')

    def test_probe_branch_git_branch(self):
        """
        Tests the value of GIT_BRANCH env. var is returned
        if MXNET_BRANCH env var is not present
        """
        with patch.dict('os.environ', {'GIT_BRANCH': 'master'}):
            self.assertEqual(probe_branch(), 'master')

        with patch.dict('os.environ', {'MXNET_BRANCH': 'feature', 'GIT_BRANCH': 'master'}):
            self.assertEqual(probe_branch(), 'feature')

    @patch('artifact_repository.get_branch_from_cmd')
    def test_probe_branch_cmd(self, mock):
        """
        Tests the git commit id from the git command is returned
        if neither MXNET_BRANCH nor GIT_BRANCH env vars are set
        """
    ...
    @patch('artifact_repository.probe_branch')
    def test_probe_no_branch(self, mock):
        """
        Tests branch gets probed if not set by user
        """
        fake_args = TestArtifactRepositoryTool.create_argparse_namespace(branch=None)
        mock.return_value = 'branchybranch'
        probe(fake_args)
        mock.assert_called_once()
        self.assertEqual(fake_args.branch, 'branchybranch')

    @patch('artifact_repository.probe_branch')
    def test_probe_no_branch_failed(self, mock):
        """
        Tests script will exit if branch probe fails
        """
        fake_args = TestArtifactRepositoryTool.create_argparse_namespace(branch=None)
        mock.return_value = None
        with self.assertRaises(SystemExit):
    ...
    def test_get_s3_key_prefix(self):
        ...
        self.assertEqual(get_s3_key_prefix(fake_args), 'master/abc123/static/linux/cpu')
    ...
    def test_get_s3_key_prefix_with_subdir(self):
        ...
        self.assertEqual(get_s3_key_prefix(fake_args, subdir='subdir'), 'master/abc123/static/linux/cpu/subdir')
