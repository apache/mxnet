...
def docker_run(platform, function_name, use_nvidia, shared_mem = '500m') {
  def command = "ci/build.py --docker-registry ${env.DOCKER_CACHE_REGISTRY} %USE_NVIDIA% --platform %PLATFORM% --ccache-dir /efs-ccache --shm-size %SHARED_MEM% /work/runtime_functions.sh %FUNCTION_NAME%"
