...
def compute_rad2deg(dtype, ndim):
    A = tvm.placeholder([tvm.var() for _ in range(ndim)], name='A', dtype=dtype)
    import math
...
def rad2deg(dtype, ndim):
    s, A, B = compute_rad2deg(dtype, ndim)
...
def compute_backward_rad2deg(dtype, ndim, req):
...
def backward_rad2deg(dtype, ndim, req):
    s, out_grad, in_grad_tmp, in_grad, c_list = compute_backward_rad2deg(dtype, ndim, req)
    for t in c_list:
        axes = [axis for axis in t.op.axis]
        fused = s[t].fuse(*axes)
        s[t].parallel(fused)
