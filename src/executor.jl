...
.. class:: Executor
   ...
   An executor is a realization of a symbolic architecture defined by a :class:`Node`.
...
function Executor(hdr :: MX_ExecutorHandle, symbol :: Node,
...
.. function:: bind(sym, ctx, args; args_grad=Dict(), aux_states=Dict(), grad_req=GRAD_WRITE)
   ...
   Create an :class:`Executor` by binding a :class:`Node` to concrete :class:`NDArray`.

   :param Node sym: the network architecture describing the computation graph.
...
function bind(self :: Node, ctx :: Context, args :: Union{Vector{NDArray},Dict{Base.Symbol,NDArray}};
...
function bind(self :: Node; kwargs...)
  kwargs = Dict(kwargs)
...
function simple_bind(self :: Node, ctx :: Context; grad_req :: GRAD_REQ=GRAD_WRITE, kwargs...)
