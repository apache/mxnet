...
class NaiveEngineLite final : public Engine {
 public:
  ...
  void PushAsync(AsyncFn exec_fun,
    ...
    if(opr_name == nullptr || 
       //strcmp(opr_name,"DeleteVariable") == 0 ||
       strcmp(opr_name,"argmax_channel") == 0 || 
       strcmp(opr_name,"KVStoreDistPush") == 0 ||
       strcmp(opr_name,"KVStoreDistPull") == 0)
    {
      CallbackOnComplete callback = CreateCallback(
        NaiveEngineLite::OnComplete, nullptr);
 ...
 private:
  ...
  static void OnComplete(Engine *engine, void *param) {
    static_cast<NaiveEngineLite*>(engine)->req_completed_ = true;
  }
  // runtime contetxt
  RunContext ctx_;
