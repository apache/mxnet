...
Profiler::Profiler()
  ...
  this->profile_stat.reset(new DevStat[cpu_num_ + gpu_num_ + 1]);
  this->profile_stat->opr_exec_stats.reserve(INITIAL_SIZE);
  for (unsigned int i = 0; i < cpu_num_; ++i) {
    profile_stat.get()[i].dev_name = "cpu/" + std::to_string(i);
  }
  for (unsigned int i = 0; i < gpu_num_; ++i) {
    profile_stat.get()[cpu_num_ + i].dev_name = "gpu/" + std::to_string(i);
  }
  profile_stat.get()[cpu_num_ + gpu_num_].dev_name = "cpu pinned/";
...
OprExecStat *Profiler::AddOprStat(int dev_type, uint32_t dev_id) {
  ...
  DevStat& dev_stat = profile_stat.get()[idx];
  {
...
void Profiler::DumpProfile() {
  ...
  for (uint32_t i = 0; i < dev_num; ++i) {
    const DevStat &d = profile_stat.get()[i];
