 ...
 * \param out the handle of the executor to be created
 */
int MXExecutorTensorRTBind(SymbolHandle symbol_handle,
                           int dev_type,
                           int dev_id,
                           const mx_uint num_g2c_keys,
                           const char** g2c_keys,
  ...
  API_BEGIN();
    ...
    if (0 == provided_grad_req_list_len
      ...
      grad_req_type = "list";
      CHECK_EQ(provided_grad_req_list_len, in_arg_names.size())
        << "The length of grad_req list does not match the number of input arguments in "
           "simple_bind, expected " << in_arg_names.size() << ", provided " <<
           provided_grad_req_list_len;
    } else if (provided_grad_req_list_len > 0
               && nullptr != provided_grad_req_names
               && nullptr != provided_grad_req_types) {  // dict, grad_req=['lhs': 'null', 'rhs':
                                                         // 'write']
      grad_req_type = "dict";
      provided_grad_req_map.reserve(provided_grad_req_list_len);
      for (mx_uint i = 0; i < provided_grad_req_list_len; ++i) {
