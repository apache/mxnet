 ...
 public:
  ...
  bool InferShape(mxnet::ShapeVector* in_shape,
    ...
    if (!shape_is_known(lshape))
      return false;
    CHECK_EQ(lshape.ndim(), 4U) << "Sampler grid should be 4D in batch-2-y-x";
    CHECK_EQ(dshape[0], lshape[0]);
    CHECK_EQ(lshape[1], 2U) << "incorrect grid shape[1], should be 2";
    // target height
    CHECK_GT(lshape[2], 0U) << "incorrect grid_shape: " << lshape[2];
    // target width
    CHECK_GT(lshape[3], 0U) << "incorrect grid_shape: " << lshape[3];
    out_shape->clear();
  ...
  bool InferType(std::vector<int>* in_type,
    ...
    for (int type : *in_type) {
      if (dtype == -1) {
        ...
        CHECK(type == dtype || type == -1) << "Non-uniform data type in BilinearSampler";
      }
    }
    if (dtype == -1) {
  ...
  std::vector<int> DeclareBackwardDependency(const std::vector<int>& out_grad,
    ...
    return {out_grad[bs::kOut], in_data[bs::kData], out_data[bs::kTmp], in_data[bs::kGrid]};
  }
