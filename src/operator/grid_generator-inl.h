...
struct GridGeneratorParam : public dmlc::Parameter<GridGeneratorParam> {
  ...
  DMLC_DECLARE_PARAMETER(GridGeneratorParam) {
    ...
    DMLC_DECLARE_FIELD(transform_type)
        ...
        .describe(
            "The type of transformation. For `affine`, input data should be an affine matrix "
  ...
  virtual void Forward(const OpContext& ctx,
    ...
    switch (param_.transform_type) {
      case grid::kAffine: {
        ...
        Tensor<xpu, 2, DType> out = out_data[grid::kOut].get_with_shape<xpu, 2, DType>(
            Shape2(out_data[grid::kOut].shape_[0] * 2,
                   out_data[grid::kOut].shape_[2] * out_data[grid::kOut].shape_[3]),
            s);
        ...
        Tensor<xpu, 2, DType> data =
            in_data[grid::kData].get_with_shape<xpu, 2, DType>(data_shape, s);
        // x, y, 1
        grid_dst[0] = range<DType>(0, grid_dst.shape_[1]);
        grid_dst[0] =
            grid_dst[0] -
            tcast<DType>(tcast<int>(grid_dst[0] / scalar<DType>(param_.target_shape[1]))) *
                scalar<DType>(param_.target_shape[1]);
        grid_dst[0] =
            scalar<DType>(-1.0) + grid_dst[0] * scalar<DType>(2.0 / (param_.target_shape[1] - 1));
        grid_dst[1] = range<DType>(0, grid_dst.shape_[1]);
        grid_dst[1] =
            scalar<DType>(-1.0) +
            tcast<DType>(tcast<int>(grid_dst[1] / scalar<DType>(param_.target_shape[1]))) *
      ...
      case grid::kWarp: {
        ...
        Tensor<xpu, 2, DType> workspace =
            ctx.requested[grid::kTempSpace].get_space_typed<xpu, 2, DType>(Shape2(2, 1), s);
    ...
    switch (param_.transform_type) {
      case grid::kAffine: {
        ...
        Tensor<xpu, 2, DType> gdata =
            in_grad[grid::kData].get_with_shape<xpu, 2, DType>(data_shape, s);
        ...
        Tensor<xpu, 2, DType> grad =
            out_grad[grid::kOut].get_with_shape<xpu, 2, DType>(grad_shape, s);
        // Legacy approach shown here for comparison:
...
class GridGeneratorProp : public OperatorProperty {
 public:
  ...
  bool InferShape(mxnet::ShapeVector* in_shape,
    ...
    switch (param_.transform_type) {
      case grid::kAffine: {
        CHECK_EQ(lshape.ndim(), 2U) << "if transform_type is affine, data is affine matrix"
      ...
      case grid::kWarp: {
        CHECK_EQ(lshape.ndim(), 4U) << "if transform_type is warp, data is optical flow"
  ...
  bool InferType(std::vector<int>* in_type,
    ...
    for (int type : *in_type) {
      if (dtype == -1) {
        ...
        CHECK(type == dtype || type == -1) << "Non-uniform data type in GridGenerator";
      }
    }
    if (dtype == -1) {
  ...
  std::vector<int> DeclareBackwardDependency(const std::vector<int>& out_grad,
    ...
    switch (param_.transform_type) {
      case grid::kAffine: {
        return {out_grad[grid::kOut], out_data[grid::kGridDst]};
      }
      case grid::kWarp: {
