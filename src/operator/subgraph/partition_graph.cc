...
bool LabelSubgraph(const nnvm::Graph& g, SubgraphSelectorV2Ptr subgraph_selector, const int label,
                   const size_t snid, const std::vector<SimpleNodePtr>& simple_nodes,
                   std::vector<SimpleNode*>* subgraph_nodes,
                   std::unordered_set<const SimpleNode*>* excluded_nodes) {
  const auto& indexed_graph = g.indexed_graph();
  ...
  struct hash_pair {
    inline size_t operator()(const std::pair<const SimpleNode*, const SimpleNode*>& v) const {
  ...
  auto is_ancestor = [&](const nnvm::Node* ancestor, const nnvm::Node* descendant,
                         const std::vector<SimpleNode*>& snodes) {
  ...
  auto sim_node_cmp = [&] (const SimpleNode* node1, const SimpleNode* node2) {
...
void SelectSubgraphNodes(nnvm::Graph* g, SubgraphSelectorV2Ptr subgraph_selector,
                         ...
                         const SimpleNode* node, const size_t snid, size_t* subgraph_id) {
  const auto& indexed_graph = g->indexed_graph();
  auto node_cmp = [&] (const SimpleNode* node1, const SimpleNode* node2) {
    return indexed_graph.node_id(node1->node) < indexed_graph.node_id(node2->node);
  };
  if (simple_nodes[snid]->label == -1 && subgraph_selector->Select(*node)) {
    // pre-select nodes that can be grouped in a subgraph
    std::vector<SimpleNode*> preselected_nodes;
    PreSelectSubgraphNodes(*g, subgraph_selector, *subgraph_id, snid, simple_nodes,
                           &preselected_nodes);

    // filter out unqualified pre-selected nodes
    std::vector<SimpleNode*> filtered_nodes = subgraph_selector->Filter(preselected_nodes);
...
nnvm::Graph PartitionGraph(nnvm::Graph&& g) {
