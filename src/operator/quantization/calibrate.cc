...
std::vector<float> SmoothDistribution(const std::vector<float>& p, const float eps = 0.0001) {
  ...
  std::vector<size_t> is_nonzeros(p.size());
  {
    auto it = p.begin();
    std::generate(
        is_zeros.begin(), is_zeros.end(), [&it]() { return static_cast<size_t>(*(it++) == 0.f); });
  }
  {
    auto it = p.begin();
    std::generate(is_nonzeros.begin(), is_nonzeros.end(), [&it]() {
...
static float ComputeEntropy(std::vector<float>* p_ptr, std::vector<float>* q_ptr) {
  ...
  for (size_t i = 0; i < p.size(); i++) {
    CHECK(p[i] > 0 && q[i] > 0);
    if (p[i] && q[i])
      ret += p[i] * std::log(p[i] / q[i]);
  }
  return ret;
}

void CalibrateComputeCPU(const nnvm::NodeAttrs& attrs,
                         const OpContext& ctx,
...
static inline bool CalibrateShape(const nnvm::NodeAttrs& attrs,
...
static inline bool CalibrateType(const nnvm::NodeAttrs& attrs,
