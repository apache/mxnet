...
void MKLDNNElementWiseSumCompute(const nnvm::NodeAttrs &attrs,
...
#if 1
  {
    ...
    auto printTensor =
        [](const std::string &name, const mshadow::Tensor<xpu, 1, DType> &t) {
    ...
    mshadow::Tensor<xpu, 1, DType>
        outdata0 = out_data[0].FlatTo1D<xpu, DType>(s);
...
//                                    memory::format::nchw};
  ...
  Tensor<xpu, 4, DType> outTensor = mkl_experimental_direct_get<xpu, 4, DType>
      (out_data[0], s);
