...
inline bool RegressionOpShape(const nnvm::NodeAttrs& attrs,
  ...
  if (lshape.ndim() == -1) {
    ...
    os << "Shape inconsistent, Provided=" << lshape << ',' << " inferred shape=" << dshape;
...
inline bool RegressionInferStorageType(const nnvm::NodeAttrs& attrs,
  ...
  if (!is_forward)
    type_assign(&out_attrs->at(1), kDefaultStorage);
...
struct DnsCsrSparseKernel {
  ...
  MSHADOW_XINLINE static void Map(int i,
    ...
    for (nnvm::dim_t j = csr_indptr[i]; j < csr_indptr[i + 1]; j++) {
      KERNEL_ASSIGN(
          out_data[row_i + csr_idx[j]], req, OP::Map(dns_data[row_i + csr_idx[j]], csr_data[j]));
    }
  }
};

template <typename xpu, typename ForwardOp>
inline void RegressionForwardImpl(mshadow::Stream<xpu>* s,
                                  const OpReqType req,
                                  const TBlob& data,
                                  const TBlob& out) {
  if (req == kNullOp)
  ...
  MSHADOW_REAL_TYPE_SWITCH(data.type_flag_, DType, {
    MXNET_ASSIGN_REQ_SWITCH(req, Req, {
      ...
      Kernel<op_with_req<ForwardOp, Req>, xpu>::Launch(s, out.Size(), out_data, in_data);
    });
  });
}
...
void RegressionForward(const nnvm::NodeAttrs& attrs,
  ...
  RegressionForwardImpl<xpu, ForwardOp>(
      s, req[reg_enum::kOut], inputs[reg_enum::kData], outputs[reg_enum::kOut]);
}
...
void RegressionForwardEx(const nnvm::NodeAttrs& attrs,
  ...
  mshadow::Stream<xpu>* s = ctx.get_stream<xpu>();
  RegressionForwardImpl<xpu, ForwardOp>(
      s, req[reg_enum::kOut], inputs[reg_enum::kData].data(), outputs[reg_enum::kOut].data());
}
    ...
    MSHADOW_REAL_TYPE_SWITCH(label.dtype(), DType, {
      MXNET_ASSIGN_REQ_SWITCH(req, Req, {
        ...
        if (req != kWriteInplace) {
          Kernel<op_with_req<mshadow_op::identity, Req>, xpu>::Launch(
              s, dshape.Size(), grad_ptr, data_ptr);
        }
        Kernel<DnsCsrSparseKernel<BackwardOp, Req>, xpu>::Launch(
            s, num_rows, grad_ptr, data_ptr, label_data, label_idx, label_indptr, row_length);
        Kernel<op_with_req<mshadow_op::mul, Req>, xpu>::Launch(
            s,
            dshape.Size(),
            grad_ptr,
            grad_ptr,
            static_cast<DType>(param.grad_scale / row_length));
      });
    });
  });
}
...
void RegressionBackwardEx(const nnvm::NodeAttrs& attrs,
  ...
  if (data_stype == kDefaultStorage && label_stype == kCSRStorage) {
    RegressionBackwardCSRImpl<xpu, BackwardOP>(s, param, req[0], inputs[1], inputs[0], outputs[0]);
  } else {
