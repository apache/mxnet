...
__global__ void image_2d_pad_edge_kernel(Tensor<gpu, 4, DType> dst,
  ...
  int inputPointX =
      min(max(padL, outputPointX), (int)src.size(3) + padL - 1) - oStartX + iStartX;
  int inputPointY =
      min(max(padT, outputPointY), (int)src.size(2) + padT - 1) - oStartY + iStartY;
...
__global__ void image_2d_pad_edge_grad_kernel(
  ...
  int oStartX = max(0, padL);
  int oStartY = max(0, padT);
  int inputPointX = min(max(padL, outputPointX), (int)grad_in.size(3) + padL - 1) -
                    oStartX + iStartX;
  int inputPointY = min(max(padT, outputPointY), (int)grad_in.size(2) + padT - 1) -
                    oStartY + iStartY;
...
__global__ void image_3d_pad_edge_kernel(Tensor<gpu, 5, DType> dst,
  ...
  int inputPointX =
      min(max(padL, outputPointX), (int)src.size(4) + padL - 1) - oStartX + iStartX;
  int inputPointY =
      min(max(padT, outputPointY), (int)src.size(3) + padT - 1) - oStartY + iStartY;
  int inputPointZ =
      min(max(padF, outputPointZ), (int)src.size(2) + padF - 1) - oStartZ + iStartZ;
...
__global__ void image_3d_pad_edge_grad_kernel(
  ...
  int inputPointX = min(max(padL, outputPointX), (int)grad_in.size(4) + padL - 1) -
                    oStartX + iStartX;
  int inputPointY = min(max(padT, outputPointY), (int)grad_in.size(3) + padT - 1) -
                    oStartY + iStartY;
  int inputPointZ = min(max(padF, outputPointZ), (int)grad_in.size(2) + padF - 1) -
                    oStartZ + iStartZ;
  DType valueToCopy =
