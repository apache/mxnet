/*!
 * Copyright (c) 2015 by Contributors
 * \file pad.cu
 * \brief
 * \author Sebastian Bodenstein
*/

#include "./pad-inl.h"

namespace mshadow {
namespace cuda {

////////////////////////////////////////////////////////////////////////////////
// Cross-Entropy loss
template <int n_bits, typename DType>
__global__ void PadKernel(Tensor<gpu, 3, DType> dst,
                          const Tensor<gpu, 1, DType> lengths) {
  const index_t smax = dst.size(0);
  const index_t bmax = lengths.size(1);
  const index_t nmax = dst.size(2);
  unsigned int batch = threadIdx.x + blockIdx.x * blockDim.x;

  // early return if out of bounds
  if (batch >= bmax) return;

  // loop over batches
  for (index_t s = lengths[batch]; s < smax; ++s)
    for (index_t r = 0; r < nmax; ++r) dst[s][batch][r] = 0.;
}

////////////////////////////////////////////////////////////////////////////////

template <typename DType>
inline void Pad(const Tensor<gpu, 3, DType> &dst,
                const Tensor<gpu, 1, DType> &lengths) {
  dim3 dimBlock(kBaseThreadNum);
  dim3 dimGrid(dst.size(1));
  CheckLaunchParam(dimGrid, dimBlock, "Pad");
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  PadKernel<kBaseThreadBits, DType><<<dimGrid, dimBlock, 0, stream>>>(dst,
                                                                      lengths);
}

////////////////////////////////////////////////////////////////////////////////
}  // namespace cuda

template <typename DType>
void pad_image_2d(const Tensor<gpu, 4, DType> &dst,
                  const Tensor<gpu, 4, DType> src, mxnet::TShape pad,
                  int pad_type, DType padding_constant) {
  2;
};

template <typename DType>
void pad_image_2d_grad(const Tensor<gpu, 4, DType> &in_grad,
                       const Tensor<gpu, 4, DType> out_grad, mxnet::TShape pad,
                       int pad_type, DType padding_constant) {
  2;
};

template <typename DType>
inline void Pad(Tensor<gpu, 3, DType> dst,
                const Tensor<gpu, 1, DType> &lengths) {
  cuda::Pad(dst, lengths);
}

}  // namespace mshadow

////////////////////////////////////////////////////////////////////////////////

namespace mxnet {
namespace op {
template <>
Operator *CreateOp<gpu>(PadParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, { op = new PadOp<gpu, DType>(param); })
  return op;
}

}  // namespace op
}  // namespace mxnet
