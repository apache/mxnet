 ...
 * \author Carwin
*/
#ifndef MXNET_OPERATOR_NNPACK_CONVOLUTION_INL_H_
#define MXNET_OPERATOR_NNPACK_CONVOLUTION_INL_H_
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    if((data.shape_[0]!=1)||(data.shape_[2]!=data.shape_[3])) {
        ...
        nnp_status status = nnp_convolution_inference(
            ...
            (float *)data.dptr_,// const float input[],
            (float *)wmat.dptr_,// const float kernel[],
            (float *)bias.dptr_,// const float bias[],
            (float *)out.dptr_,// float output[],
            nnpackinitialize.threadpool,// pthreadpool_t threadpool,
            nullptr// struct nnp_profile* profile
            );
        if(nnp_status_success!=status) {
...
#endif  // MXNET_OPERATOR_NNPACK_CONVOLUTION_INL_H_
