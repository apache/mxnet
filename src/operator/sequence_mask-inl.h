 ...
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    if (param_.use_sequence_length) {
      Tensor<xpu, 1, DType> indices = in_data[seq_mask::kSequenceLength].get<xpu, 1, DType>(s);
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Assign(data_grad, req[seq_mask::kData], F<mshadow_op::identity>(output_grad));
...
class SequenceMaskProp : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    if (param_.use_sequence_length)
      SHAPE_ASSIGN_CHECK(*in_shape, seq_mask::kSequenceLength, Shape1(dshape[1]));
  ...
  std::vector<int> DeclareBackwardDependency(
    ...
    if (param_.use_sequence_length)
      return {out_grad[seq_mask::kOut],
            in_data[seq_mask::kSequenceLength]
           };
    else
      return {out_grad[seq_mask::kOut]};
  }
