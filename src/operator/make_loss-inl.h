...
struct MakeLossParam : public dmlc::Parameter<MakeLossParam> {
  ...
  DMLC_DECLARE_PARAMETER(MakeLossParam) {
    ...
    DMLC_DECLARE_FIELD(valid_thresh)
        .set_default(0.0f)
        .describe(
            "clip each element in the array to 0 when it is less than ``valid_thresh``."
  ...
  virtual void Backward(const OpContext& ctx,
    ...
    if (param_.normalization == make_loss_enum::kValid) {
      ...
      Tensor<xpu, 1, DType> temp =
          ctx.requested[make_loss_enum::kTempSpace].get_space_typed<xpu, 1, DType>(
              mshadow::Shape1(1), s);
      ...
      Assign(grad,
             req[make_loss_enum::kData],
             ScalarExp<DType>(param_.grad_scale) /
                 broadcast<0>(broadcast_keepdim(temp, 0, grad.shape_[0]), grad.shape_));
    } else if (param_.normalization == make_loss_enum::kBatch) {
      Assign(
          grad, req[make_loss_enum::kData], ScalarExp<DType>(param_.grad_scale / grad.shape_[0]));
    } else {
