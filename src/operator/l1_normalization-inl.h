...
struct L1NormalizationParam : public dmlc::Parameter<L1NormalizationParam> {
  float eps;
  int mode;
...
public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    if (param_.mode == l1_normalization::kInstance) {
      ...
      Tensor<xpu, 2, DType> data = in_data[l1_normalization::kData]
        .get_with_shape<xpu, 2, DType>(dshape, s);
      Tensor<xpu, 2, DType> out = out_data[l1_normalization::kOut]
        .get_with_shape<xpu, 2, DType>(dshape, s);
      Tensor<xpu, 1, DType> norm = out_data[l1_normalization::kNorm].get<xpu, 1, DType>(s);
      norm = sumall_except_dim<0>(F<mxnet::op::mshadow_op::abs>(data));
      ...
      Tensor<xpu, 3, DType> data = in_data[l1_normalization::kData]
        .get_with_shape<xpu, 3, DType>(dshape, s);
      Tensor<xpu, 3, DType> out = out_data[l1_normalization::kOut]
        .get_with_shape<xpu, 3, DType>(dshape, s);
      Shape<2> norm_shape = Shape2(dshape[0], dshape[2]);
      Tensor<xpu, 2, DType> norm = out_data[l1_normalization::kNorm]
        .get_with_shape<xpu, 2, DType>(norm_shape, s);
      norm = reduce_with_axis<red::sum, false>(F<mxnet::op::mshadow_op::abs>(data), 1);
      MXNET_ASSIGN_REQ_SWITCH(req[0], Req, {
        mxnet_op::Kernel<mxnet_op::op_with_req<mxnet::op::mshadow_op::plus, Req>, xpu>::Launch(
      ...
      Tensor<xpu, 2, DType> norm = out_data[l1_normalization::kNorm]
        .get_with_shape<xpu, 2, DType>(norm_shape, s);
      norm = reduce_with_axis<red::sum, false>(F<mxnet::op::mshadow_op::abs>(data), 2);
      MXNET_ASSIGN_REQ_SWITCH(req[0], Req, {
        mxnet_op::Kernel<mxnet_op::op_with_req<mxnet::op::mshadow_op::plus, Req>, xpu>::Launch(
          s, norm.size(0) * norm.size(1), norm.dptr_, norm.dptr_, DType(param_.eps));
      });
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Stream<xpu> *s = ctx.get_stream<xpu>();
    TShape orig_shape = out_data[l1_normalization::kOut].shape_;
    if (param_.mode == l1_normalization::kInstance) {
      Shape<2> dshape = Shape2(orig_shape[0],
        orig_shape.ProdShape(1, orig_shape.ndim()));
      Tensor<xpu, 2, DType> data = out_data[l1_normalization::kOut]
        .get_with_shape<xpu, 2, DType>(dshape, s);
      Tensor<xpu, 2, DType> grad_in = in_grad[l1_normalization::kData]
        .get_with_shape<xpu, 2, DType>(dshape, s);
      Tensor<xpu, 2, DType> grad_out = out_grad[l1_normalization::kOut]
        .get_with_shape<xpu, 2, DType>(dshape, s);
      Tensor<xpu, 1, DType> norm = out_data[l1_normalization::kNorm].get<xpu, 1, DType>(s);
      Tensor<xpu, 1, DType> temp = ctx.requested[l1_normalization::kTempSpace]
        .get_space_typed<xpu, 1, DType>(mshadow::Shape1(data.shape_[0]), s);
      Assign(grad_in, req[l1_normalization::kData],
        (grad_out) / broadcast<0>(norm, data.shape_));
    } else if (param_.mode == l1_normalization::kChannel) {
      CHECK_GE(orig_shape.ndim(), 3U);
      Shape<3> dshape = Shape3(orig_shape[0], orig_shape[1],
        orig_shape.ProdShape(2, orig_shape.ndim()));
      Tensor<xpu, 3, DType> data = out_data[l1_normalization::kOut]
        .get_with_shape<xpu, 3, DType>(dshape, s);
      Tensor<xpu, 3, DType> grad_in = in_grad[l1_normalization::kData]
        .get_with_shape<xpu, 3, DType>(dshape, s);
      Tensor<xpu, 3, DType> grad_out = out_grad[l1_normalization::kOut]
        .get_with_shape<xpu, 3, DType>(dshape, s);
      Shape<2> norm_shape = Shape2(dshape[0], dshape[2]);
      Tensor<xpu, 2, DType> norm = out_data[l1_normalization::kNorm]
        .get_with_shape<xpu, 2, DType>(norm_shape, s);
 ...
 private:
  L1NormalizationParam param_;
};  // class L1NormalizationOp

// Decalre Factory function, used for dispatch specialization
template<typename xpu>
Operator* CreateOp(L1NormalizationParam param, int dtype);

#if DMLC_USE_CXX11
class L1NormalizationProp : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    using namespace mshadow;
    CHECK_EQ(in_shape->size(), 1U) << "L1Normalization layer only accepts data as input";
    const TShape &dshape = (*in_shape)[l1_normalization::kData];
    // require data to be known
    if ((*in_shape)[l1_normalization::kData].ndim() == 0) return false;
    out_shape->clear();
  ...
  OperatorProperty* Copy() const override {
    L1NormalizationProp* norm_sym = new L1NormalizationProp();
  ...
  std::vector<int> DeclareBackwardDependency(
    ...
    return {out_grad[l1_normalization::kOut],
      out_data[l1_normalization::kOut],
      out_data[l1_normalization::kNorm]};
  }
  ...
  std::vector<std::pair<int, void*> > BackwardInplaceOption(
    ...
    return {{out_grad[l1_normalization::kOut], in_grad[l1_normalization::kData]}};
  }
 ...
 private:
  L1NormalizationParam param_;
};  // class L1NormalizationSymbol
#endif
}  // namespace op
}  // namespace mxnet
#endif  // MXNET_OPERATOR_L1_NORMALIZATION_INL_H_
