...
static std::vector<nnvm::NodeEntry> ForeachGradient(const nnvm::ObjectPtr& n,
  ...
  CHECK_EQ(p->num_inputs(), p->inputs.size())
      << "Number of inputs to operator "
      << "_backward_npx_foreach"
      << " (" << p->num_inputs()
      << ") does not match the actual number of inputs provided to operator " << n->attrs.name
      << " (" << p->inputs.size() << ").";
...
static bool WhileLoopShape(const nnvm::NodeAttrs& attrs,
  ...
  bool infer_success = InferSubgraphShape(*attrs.subgraphs[1], &subg_in_shape, &subg_out_shape);
  for (int i = 0; i < params.num_out_data; i++) {
    // If the output shape isn't inferred, we don't need to propogate the info.
    const auto& g_out_shape = subg_out_shape[i];
    if (!mxnet::ndim_is_known(g_out_shape))
...
NNVM_REGISTER_OP(_npx_foreach)
    ...
    .set_attr<nnvm::FListInputNames>("FListInputNames",
                                     [](const NodeAttrs& attrs) {
                                       const NPXForeachParam& params =
                                           nnvm::get<NPXForeachParam>(attrs.parsed);
...
NNVM_REGISTER_OP(_npx_while_loop)
    ...
    .set_attr<nnvm::FListInputNames>("FListInputNames",
                                     [](const NodeAttrs& attrs) {
                                       const NPXWhileLoopParam& params =
                                           nnvm::get<NPXWhileLoopParam>(attrs.parsed);
                                       std::vector<std::string> names;
...
NNVM_REGISTER_OP(_npx_cond)
    ...
    .set_attr<nnvm::FListInputNames>("FListInputNames",
                                     [](const NodeAttrs& attrs) {
                                       const NPXCondParam& params =
                                           nnvm::get<NPXCondParam>(attrs.parsed);
                                       std::vector<std::string> names;
                                       names.reserve(params.num_args);
                                       names.emplace_back("cond");
