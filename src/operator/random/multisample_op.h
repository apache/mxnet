...
inline bool MultiSampleOpShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_GT(in_attrs->size(), 0) << "sampling operator takes 1 or 2 arguments (" << in_attrs->size()
                                << " given)";
  CHECK_LT(in_attrs->size(), 3) << "sampling operator takes 1 or 2 arguments (" << in_attrs->size()
                                << " given)";
  ...
  for (const auto& in_attr : *in_attrs) {
    if (!shape_assign(&tshape, in_attr))
      return false;
  }
...
inline bool MultiSampleOpType(const nnvm::NodeAttrs& attrs,
  ...
  if (dtype != -1) {
    if (param.dtype != -1) {
      ...
      CHECK_EQ(dtype, param.dtype)
          << "Inferred output type does not match requested type: " << dtype << " vs "
          << param.dtype;
    }
  } else {
  ...
  CHECK_EQ(dtype_ok, true) << "Output type must be float16, float32, or float64: dtype is " << dtype
...
struct SamplerCaller<xpu, IType, OType, Sampler, 1> {
  static void op(const std::vector<TBlob>& inputs,
    ...
    Sampler sampler;
    sampler.Sample(inputs[0].FlatTo1D<xpu, IType>(s), outputs[0].FlatTo1D<xpu, OType>(s), pgen, s);
  }
};
