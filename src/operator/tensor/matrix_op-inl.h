...
struct Transpose2D{
  ...
  MSHADOW_XINLINE static void Map(const DType *in, DType *out, index_t row, index_t col) {
    ...
    switch (axes.ndim()) {
     ...
     case 2: {
      ...
      if (axes[0] == 1 && axes[1] == 0) {
        ...
        Kernel<Transpose2D, xpu>::Launch(s, size, in.dptr_, out.dptr_, in.shape_[0], in.shape_[1]);
      } else {
        Copy(out, in, s);
      }
      break;
