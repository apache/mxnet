...
// a poisson-gamma mixture: X ~ NegBin(mu, alpha) corresponds to X ~ Poisson(Gamma(1/alpha,mu*alpha))
struct GeneralizedNegativeBinomialSampler {
  ...
  struct Sampler {
    ...
    template<typename PType>
    Sampler(PType mu, PType alpha, int seed): poisson_(alpha == 0.0), mu_(mu), rnd_(seed), gen_((alpha == PType(0) ? PType(1) : PType(1)/alpha),mu*alpha){};
    // Allow sampling of a Poisson distribution also to output floating point types.
    typedef typename std::conditional<std::is_integral<DType>::value,DType,int>::type GType;
    MSHADOW_XINLINE DType operator()() { return static_cast<DType>(std::poisson_distribution<GType>(poisson_ ? mu_ : gen_(rnd_))(rnd_)); }
  };
};
...
#define MXNET_OPERATOR_REGISTER_SAMPLING(distr,sampler,num_inputs,input_name_1,input_name_2,description)                  \
  ...
  .MXNET_DESCRIBE("Multi-sampling from " description "."                                                                  \
                  ...
                  " Let \"[s]\" be the shape of the input tensor(s), \"n\" be the dimension of [s], \"[t]\" be the"       \
                  " shape specified as the parameter of the operator, and \"m\" be the dimension of [t]. Then the output" \
                  " will be a (n+m)-dimensional tensor with shape [s]x[t]. For any valid n-dimensional index \"i\" with"  \
                  " respect to the input tensor(s), output[i] will be an m-dimensional tensor that holds "                \
                  " randomly drawn samples from the distribution which is parameterized by the input values at index i."  \
                  " If the shape parameter of the operator is not set, then one sample will be drawn per "                \
                  " distribution and the output tensor has same dimensions as the input tensor(s).")                      \
...
MXNET_OPERATOR_REGISTER_SAMPLING2(uniform,UniformSampler,"low","high","uniform distributions on the interval [low,high)")
MXNET_OPERATOR_REGISTER_SAMPLING2(normal,NormalSampler,"mu","sigma","normal distributions with parameters mu and sigma")
MXNET_OPERATOR_REGISTER_SAMPLING2(gamma,GammaSampler,"alpha","beta","gamma distributions with parameters alpha and beta")
MXNET_OPERATOR_REGISTER_SAMPLING1(exponential,ExponentialSampler,"lambda","exponential distributions with parameters lambda")
MXNET_OPERATOR_REGISTER_SAMPLING1(poisson,PoissonSampler,"lambda","Poisson distributions with parameters lambda")
MXNET_OPERATOR_REGISTER_SAMPLING2(negative_binomial,NegativeBinomialSampler,"k","p","negative binomial distributions with parameters k (failure limit) and p (failure probability)")
MXNET_OPERATOR_REGISTER_SAMPLING2(generalized_negative_binomial,GeneralizedNegativeBinomialSampler,"mu","alpha","generalized negative binomial distributions with parameters mu (mean) and alpha (over dispersion)")
