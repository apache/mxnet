...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_sin, unary_bwd<mshadow_op::sin_grad>)
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          // f'(x) = cos(x)
          // f''(x) = -sin(x)
          auto dydx = MakeNode("cos", n->attrs.name + "_dydx", {n->inputs[1]}, nullptr, &n);
          auto d2ydx2 =
              MakeNode("negative",
                       n->attrs.name + "_d2ydx2",
                       {nnvm::NodeEntry{MakeNode(
                           "sin", n->attrs.name + "_grad_grad_mid", {n->inputs[1]}, nullptr, &n)}},
                       nullptr,
                       &n);

          auto grad_grad_mid = MakeNode("elemwise_mul",
                                        n->attrs.name + "backward_grad_grad_mid",
          ...
          ret.emplace_back(MakeNode("elemwise_mul",
                                    ...
                                    &n));
          ret.emplace_back(MakeNode("elemwise_mul",
                                    n->attrs.name + "_backward_grad_grad_in",
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          auto dydx =
              MakeNode("negative",
                       n->attrs.name + "_dydx",
                       {nnvm::NodeEntry{MakeNode(
                           "sin", n->attrs.name + "_grad_mid", {n->inputs[1]}, nullptr, &n)}},
                       nullptr,
                       &n);
          auto d2ydx2 =
              MakeNode("negative",
                       n->attrs.name + "_d2ydx2",
                       {nnvm::NodeEntry{MakeNode(
                           "cos", n->attrs.name + "_grad_grad_mid", {n->inputs[1]}, nullptr, &n)}},
                       nullptr,
                       &n);

          auto grad_grad_mid = MakeNode("elemwise_mul",
                                        n->attrs.name + "_backward_grad_grad_mid",
                                        {n->inputs[0], nnvm::NodeEntry{d2ydx2}},
                                        nullptr,
                                        &n);
    ...
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          auto grad_grad_mid = MakeNode("elemwise_mul",
                                        ...
                                        {n->inputs[0], nnvm::NodeEntry{two_y}},
                                        nullptr,
                                        &n);
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_arcsin,
    ...
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          auto x_grad              = nnvm::NodeEntry{mxnet::op::MakeNode(
              "_backward_arcsin", n->attrs.name + "_backward_arcsin", {ones, x}, nullptr, &n)};
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_arccos,
    ...
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          auto ones                = op.ones_like(x);
          auto x_grad              = nnvm::NodeEntry{mxnet::op::MakeNode(
              "_backward_arccos", n->attrs.name + "_backward_arccos", {ones, x}, nullptr, &n)};
          auto x_grad_square       = op.square(x_grad);
          auto x_grad_square_mul_x = op.mul(x_grad_square, x);
          auto x_grad_grad         = op.mul(dydx_mul_grad_x, x_grad_square_mul_x);
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_arctan,
    ...
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          auto ones                  = op.ones_like(x);
          auto x_grad                = nnvm::NodeEntry{mxnet::op::MakeNode(
              "_backward_arctan", n->attrs.name + "_backward_arctan", {ones, x}, nullptr, &n)};
          auto x_grad_square         = op.square(x_grad);
          auto x_grad_square_mul_x   = op.mul(x_grad_square, x);
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_sinh, unary_bwd<mshadow_op::sinh_grad>)
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          // f'(x) = cosh(x)
          // f''(x) = sinh(x)
          auto dydx = MakeNode("cosh", n->attrs.name + "_dydx", {n->inputs[1]}, nullptr, &n);
          auto d2ydx2 =
              MakeNode("sinh", n->attrs.name + "_grad_grad_mid", {n->inputs[1]}, nullptr, &n);
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU(_backward_cosh, unary_bwd<mshadow_op::cosh_grad>)
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          // f'(x) = sinh(x)
          // f''(x) = cosh(x)
          auto dydx = MakeNode("sinh", n->attrs.name + "_dydx", {n->inputs[1]}, nullptr, &n);
          auto d2ydx2 =
              MakeNode("cosh", n->attrs.name + "_grad_grad_mid", {n->inputs[1]}, nullptr, &n);
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_tanh, unary_bwd<mshadow_op::tanh_grad>)
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          const std::unordered_map<std::string, std::string> args = {{"scalar", "-2.0"}};
          auto neg_two_y =
              MakeNode("_mul_scalar", n->attrs.name + "_mul_neg_two", {n->inputs[1]}, &args, &n);
          auto grad_grad_mid = MakeNode("elemwise_mul",
                                        n->attrs.name + "_grad_mul",
                                        {n->inputs[0], nnvm::NodeEntry{neg_two_y}},
                                        nullptr,
                                        &n);
          auto dydx          = MakeNode("elemwise_div",
                               n->attrs.name + "_grad_div",
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_arcsinh,
    ...
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          auto ones                = op.ones_like(x);
          auto grad_x              = nnvm::NodeEntry{mxnet::op::MakeNode(
              "_backward_arcsinh", n->attrs.name + "_backward_arcsinh", {ones, x}, nullptr, &n)};
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_arccosh,
    ...
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          auto ones                = op.ones_like(x);
          auto grad_x              = nnvm::NodeEntry{mxnet::op::MakeNode(
              "_backward_arccosh", n->attrs.name + "_backward_arccosh", {ones, x}, nullptr, &n)};
          auto grad_x_square       = op.square(grad_x);
          auto grad_x_square_mul_x = op.mul(grad_x_square, x);
          auto grad_grad_x         = op.mul(dydx_mul_grad_x, grad_x_square_mul_x);
...
MXNET_OPERATOR_REGISTER_BINARY_WITH_SPARSE_CPU_DR(_backward_arctanh,
    ...
    .set_attr<nnvm::FGradient>(
        ...
        [](const nnvm::ObjectPtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
          ...
          auto ones                  = op.ones_like(x);
          auto x_grad                = nnvm::NodeEntry{mxnet::op::MakeNode(
              "_backward_arctanh", n->attrs.name + "_backward_arctanh", {ones, x}, nullptr, &n)};
          auto x_grad_square         = op.square(x_grad);
          auto x_grad_square_mul_x   = op.mul(x_grad_square, x);
          auto x_grad_square_mul_2_x = op.mul(2.0, x_grad_square_mul_x);
          auto grad_grad_x           = op.mul(dldy, x_grad_square_mul_2_x);
