...
.attr<nnvm::FGradient>("FGradient", ElemwiseGradUseNone{"_backward_sum"});

MXNET_OPERATOR_REGISTER_REDUCE_BACKWARD(_backward_sum)
.set_num_inputs(1)
.attr<FCompute>("FCompute<cpu>", ReduceAxesBackwardUseNone<cpu>);

MXNET_OPERATOR_REGISTER_REDUCE(max)
.MXNET_DESCRIBE("Compute max along axis. If axis is empty, global reduction is performed")
.attr<FCompute>("FCompute<cpu>", ReduceAxesCompute<cpu, mshadow::red::maximum>)
.attr<nnvm::FGradient>("FGradient", ReduceGrad{"_backward_max"});

MXNET_OPERATOR_REGISTER_REDUCE_BACKWARD(_backward_max)
.set_num_inputs(3)
.attr<FCompute>("FCompute<cpu>", ReduceAxesBackwardUseInOut<cpu, mshadow_op::eq>);

MXNET_OPERATOR_REGISTER_REDUCE(min)
.MXNET_DESCRIBE("Compute min along axis. If axis is empty, global reduction is performed")
.attr<FCompute>("FCompute<cpu>", ReduceAxesCompute<cpu, mshadow::red::minimum>)
.attr<nnvm::FGradient>("FGradient", ReduceGrad{"_backward_min"});
...
.attr<nnvm::FInferShape>("FInferShape", BroadcastAxesShape)
...
.attr<nnvm::FInferShape>("FInferShape", BroadcastToShape)
.attr<FCompute>("FCompute<cpu>", BroadcastCompute<cpu>);
