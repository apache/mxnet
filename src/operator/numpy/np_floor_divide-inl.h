...
/*!
 * \file np_floor_divide-inl.h
 * \brief Function definitions of floor_divide operator
 */

#ifndef MXNET_OPERATOR_NUMPY_NP_FLOOR_DIVIDE_INL_H_
#define MXNET_OPERATOR_NUMPY_NP_FLOOR_DIVIDE_INL_H_
...
void FloorDivideScalarCompute(const nnvm::NodeAttrs& attrs,
                              const OpContext& ctx,
...
void FloorDivideElemwiseCompute(const nnvm::NodeAttrs& attrs,
                                const OpContext& ctx,
                                const std::vector<TBlob>& inputs,
                                const std::vector<OpReqType>& req,
                                const std::vector<TBlob>& outputs) {
...
template <typename xpu>
void FloorDivideBroadcastCompute(const nnvm::NodeAttrs& attrs,
                                 const OpContext& ctx,
                                 const std::vector<TBlob>& inputs,
                                 const std::vector<OpReqType>& req,
                                 const std::vector<TBlob>& outputs) {
  ...
  if (!ndim) {
    FloorDivideElemwiseCompute<xpu>(attrs, ctx, inputs, req, outputs);
  } else {
    if (req[0] == kNullOp)
      return;
...
#endif  // MXNET_OPERATOR_NUMPY_NP_FLOOR_DIVIDE_INL_H_
