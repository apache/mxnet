...
struct DiffParam : public dmlc::Parameter<DiffParam> {
  ...
  DMLC_DECLARE_PARAMETER(DiffParam) {
    DMLC_DECLARE_FIELD(n)
      .set_default(1)
      .describe("The number of times values are differenced."
    ...
    MSHADOW_TYPE_SWITCH(out.type_flag_, OType, {
      MXNET_NDIM_SWITCH(in.ndim(), ndim, {
        Kernel<diff_forward, xpu>::Launch(
          ...
          out.dptr<OType>(), in.dptr<IType>(), n, stride, 
          out.shape_.get<ndim>(), in.shape_.get<ndim>());
      })
    });
  });
}
...
void DiffBackwardImpl(const OpContext& ctx,
  ...
  if (n >= igrad.shape_[axis_checked]) return; // nothing in the ograd and igrad
  int stride = 1;
