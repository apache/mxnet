...
inline bool NumpyLaNormShape(const nnvm::NodeAttrs& attrs,
  ...
  SHAPE_ASSIGN_CHECK(*out_attrs, 0, NumpyNormReduceAxesShapeImpl((*in_attrs)[0], param.axis, param.keepdims, false)); // NOLINT(*)
  return true;
}
...
struct numpy_norm_reduce_axes_backward_broadcast{
  ...
  MSHADOW_XINLINE static void Map(index_t i,
    ...
    if (normzero) {
      ...
      KERNEL_ASSIGN(igrad[i], req, DType(ograd[out_idx]) * DType(OP::Map(DType(flag_data*data[i]), DType(out[out_idx]))*flag_data)); // NOLINT(*)
    }
  }
};
...
void NumpyLaNormCompute(const nnvm::NodeAttrs& attrs,
  ...
  if ((flag_axis && param.axis.value().ndim() == 1) || (!flag_axis && ndim == 1)) {
    ...
    if (param.flag == 2 || (param.flag == -1 && (param.ord == 2 || param.ord == -2))) {
      ...
      LOG(FATAL) << "You must give axis, like (0, 1).because it isn't implemented in C++ backend.";
    } else {
