...
struct TensorinvParam : public dmlc::Parameter<TensorinvParam> {
  ...
  DMLC_DECLARE_PARAMETER(TensorinvParam) {
    DMLC_DECLARE_FIELD(ind).set_default(2).describe(
...
void TensorinvOpBackward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_SGL_DBL_TYPE_SWITCH(outputs[0].type_flag_, OType, {
    ...
    CHECK_LE(inv_a_shape.ndim(), 6U) << "tensorinv backward only support tensor's dimension <= 6";
    if (axes < inv_a_shape.ndim()) {
      ...
      Tensor<xpu, 1, char> workspace = ctx.requested[0].get_space_typed<xpu, 1, char>(
          Shape1(temp_shape.Size() * sizeof(OType)), ctx.get_stream<xpu>());
      TBlob temp_tblob =
      ...
      Tensor<xpu, 1, OType> inv_a_tensor    = inv_a.get_with_shape<xpu, 1, OType>(Shape1(1), s);
      Tensor<xpu, 1, OType> out_grad_tensor = out_grad.get_with_shape<xpu, 1, OType>(Shape1(1), s);
      Tensor<xpu, 1, OType> grad_a_tensor   = grad_a.get_with_shape<xpu, 1, OType>(Shape1(1), s);
      ASSIGN_DISPATCH(
          grad_a_tensor, kWriteTo, OType(-1) * inv_a_tensor * out_grad_tensor * inv_a_tensor);
    }
  });
}
