...
struct BooleanAssignCPUKernel {
 private:
  static size_t bin_search(const size_t* idx, const size_t idx_size, const size_t i) {
      ...
      for (size_t t = 0; t < trailing; ++t) {
        if (scalar) {
          ...
          data[(l * middle + mid) * trailing + t] =
              (broadcast) ? tensor[l * trailing + t] : tensor[(l * valid_num + i) * trailing + t];
        }
      }
    }
  }
      ...
      if (vshape.Size() > 1) {
        for (int i = 0; i < start_axis; ++i) {
          CHECK_EQ(dshape[i], vshape[i])
              << "shape mismatch of value with input at dimension " << i << "; dimension is "
              << dshape[i] << " but corresponding value dimension is " << vshape[i];
        }
...
void NumpyBooleanAssignForwardCPU(const nnvm::NodeAttrs& attrs,
  ...
  if (inputs.size() == 3U) {
    ...
    MSHADOW_TYPE_SWITCH_WITH_BOOL(data.type_flag_, DType, {
      if (inputs[2].shape_.Size() == 1) {
        Kernel<BooleanAssignCPUKernel<true>, cpu>::Launch(s,
                                                          valid_num,
                                                          data.dptr<DType>(),
                                                          prefix_sum.data(),
                                                          prefix_sum.size(),
    ...
    MSHADOW_TYPE_SWITCH_WITH_BOOL(data.type_flag_, DType, {
      Kernel<BooleanAssignCPUKernel<true>, cpu>::Launch(
          ...
          middle,
          trailing,
          static_cast<DType>(std::stod(attrs.dict.at("value"))));
    });
  }
}
