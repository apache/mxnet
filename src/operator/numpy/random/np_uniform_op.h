...
struct uniform_two_scalar_kernel {
  MSHADOW_XINLINE static void Map(index_t i, float low, float high, float* uniform, OType* out) {
...
void NumpyUniformForward(const nnvm::NodeAttrs& attrs,
  ...
  if (inputs.size() == 0U) {
    MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, OType, {
      Kernel<uniform_two_scalar_kernel<OType>, xpu>::Launch(s,
                                                            outputs[0].Size(),
                                                            param.low.value(),
                                                            param.high.value(),
    ...
    MSHADOW_TYPE_SWITCH(inputs[0].type_flag_, IType, {
      MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, OType, {
        BROADCAST_NDIM_SWITCH(ndim, NDim, {
          ...
          Kernel<uniform_kernel<NDim, IType, OType>, xpu>::Launch(s,
                                                                  outputs[0].Size(),
                                                                  lstride,
                                                                  hstride,
                                                                  oshape,
