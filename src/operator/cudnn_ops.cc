...
cudnnDataType_t CudnnType(mshadow::TypeFlag dtype) {
  static std::unordered_map<mshadow::TypeFlag, cudnnDataType_t> type_map{
      {mshadow::kFloat32, CUDNN_DATA_FLOAT},
      {mshadow::kFloat64, CUDNN_DATA_DOUBLE},
      {mshadow::kFloat16, CUDNN_DATA_HALF},
...
std::vector<size_t> LayoutInfo::Order() const {
  ...
  if (channel_last) std::rotate(ret.begin() + 1, ret.begin() + 2, ret.end());
  return ret;
}

size_t LayoutInfo::ChannelIdx() const { return channel_last ? 1 + n_space_dims : 1; }
...
std::vector<size_t> ReverseOrder(const std::vector<size_t>& o) {
  ...
  for (size_t i = 0; i < ret.size(); ++i) ret[o[i]] = i;
  return ret;
}
...
Descriptor MakeTensorDesc(int64_t uid, cudnnDataType_t dtype, const std::vector<int64_t>& dims,
                          const std::vector<int64_t>& strides, bool is_virtual) {
  int64_t alignment = 16;  // TODO(vcherepanov): ?
  return MakeFinalized(CUDNN_BACKEND_TENSOR_DESCRIPTOR,
                       CUDNN_ATTR_TENSOR_UNIQUE_ID, uid,
                       CUDNN_ATTR_TENSOR_DATA_TYPE, dtype,
                       CUDNN_ATTR_TENSOR_BYTE_ALIGNMENT, alignment,
                       CUDNN_ATTR_TENSOR_DIMENSIONS, dims,
                       CUDNN_ATTR_TENSOR_STRIDES, strides,
                       CUDNN_ATTR_TENSOR_IS_VIRTUAL, is_virtual);
}

Descriptor MakeTensorDesc(int64_t uid, const TBlob& blob, const LayoutInfo& li, bool expand_1d,
                          bool is_virtual) {
  std::vector<int64_t> dims(blob.shape_.ndim());
  CHECK_EQ(dims.size(), li.n_space_dims + 2);
  auto rev_order = ReverseOrder(li.Order());
  for (size_t i = 0; i < dims.size(); ++i) dims[i] = blob.shape_[rev_order[i]];
  ...
  if (li.n_space_dims == 1 && expand_1d) {
    ...
    if (li.channel_last) std::rotate(order.begin() + 1, order.begin() + 2, order.end());
    strides = PackedStrides(order, dims);
  }
  return MakeTensorDesc(uid, CudnnType(static_cast<mshadow::TypeFlag>(blob.type_flag_)), dims,
                        strides, is_virtual);
}

Descriptor MakeCTensorDescExpandDims(int64_t uid, const TBlob& b, const LayoutInfo& li,
                                     bool is_virtual) {
...
Descriptor MakeConvDesc(const ConvParam& param, mshadow::TypeFlag dtype) {
  ...
  if (comp_type == CUDNN_DATA_HALF) comp_type = CUDNN_DATA_FLOAT;
  ...
  return MakeFinalized(CUDNN_BACKEND_CONVOLUTION_DESCRIPTOR,
                       CUDNN_ATTR_CONVOLUTION_SPATIAL_DIMS, sdims,
                       CUDNN_ATTR_CONVOLUTION_COMP_TYPE, comp_type,
                       CUDNN_ATTR_CONVOLUTION_CONV_MODE, CUDNN_CROSS_CORRELATION,
                       CUDNN_ATTR_CONVOLUTION_FILTER_STRIDES, stride,
                       CUDNN_ATTR_CONVOLUTION_DILATIONS, dilate,
                       CUDNN_ATTR_CONVOLUTION_PRE_PADDINGS, pad,
                       CUDNN_ATTR_CONVOLUTION_POST_PADDINGS, pad);
}

Descriptor MakeConvFwdOp(const Descriptor& conv, const Descriptor& x, const Descriptor& w,
                         const Descriptor& y, bool add_to) {
  auto ret = Make(CUDNN_BACKEND_OPERATION_CONVOLUTION_FORWARD_DESCRIPTOR,
                  ...
                  CUDNN_ATTR_OPERATION_CONVOLUTION_FORWARD_X, x,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_FORWARD_W, w,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_FORWARD_Y, y);
  if (GetAttr<cudnnDataType_t>(x, CUDNN_ATTR_TENSOR_DATA_TYPE) == CUDNN_DATA_DOUBLE) {
    SetAttrs(ret,
             CUDNN_ATTR_OPERATION_CONVOLUTION_FORWARD_ALPHA, 1.0,
             CUDNN_ATTR_OPERATION_CONVOLUTION_FORWARD_BETA, add_to ? 1.0 : 0.0);
  } else {
    SetAttrs(ret,
             CUDNN_ATTR_OPERATION_CONVOLUTION_FORWARD_ALPHA, 1.0f,
             CUDNN_ATTR_OPERATION_CONVOLUTION_FORWARD_BETA, add_to ? 1.0f : 0.0f);
  }
  CUDNN_CALL(cudnnBackendFinalize(ret.get()));
  return ret;
}

Descriptor MakeConvDgradOp(const Descriptor& conv, const Descriptor& w, const Descriptor& dy,
                           const Descriptor& dx, bool add_to) {
  auto ret = Make(CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_DATA_DESCRIPTOR,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_DATA_CONV_DESC, conv,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_DATA_W, w,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_DATA_DY, dy,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_DATA_DX, dx);
  if (GetAttr<cudnnDataType_t>(w, CUDNN_ATTR_TENSOR_DATA_TYPE) == CUDNN_DATA_DOUBLE) {
    SetAttrs(ret,
             CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_DATA_ALPHA, 1.0,
             CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_DATA_BETA, add_to ? 1.0 : 0.0);
  } else {
    SetAttrs(ret,
             CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_DATA_ALPHA, 1.0f,
             CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_DATA_BETA, add_to ? 1.0f : 0.0f);
  }
  CUDNN_CALL(cudnnBackendFinalize(ret.get()));
  return ret;
}

Descriptor MakeConvWgradOp(const Descriptor& conv, const Descriptor& x, const Descriptor& dy,
                           const Descriptor& dw, bool add_to) {
  auto ret = Make(CUDNN_BACKEND_OPERATION_CONVOLUTION_BACKWARD_FILTER_DESCRIPTOR,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_FILTER_CONV_DESC, conv,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_FILTER_X, x,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_FILTER_DY, dy,
                  CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_FILTER_DW, dw);
  if (GetAttr<cudnnDataType_t>(x, CUDNN_ATTR_TENSOR_DATA_TYPE) == CUDNN_DATA_DOUBLE) {
    SetAttrs(ret,
             CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_FILTER_ALPHA, 1.0,
             CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_FILTER_BETA, add_to ? 1.0 : 0.0);
  } else {
    SetAttrs(ret,
             CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_FILTER_ALPHA, 1.0f,
             CUDNN_ATTR_OPERATION_CONVOLUTION_BWD_FILTER_BETA, add_to ? 1.0f : 0.0f);
  }
  CUDNN_CALL(cudnnBackendFinalize(ret.get()));
  return ret;
}
...
Descriptor MakeOpGraph(cudnnHandle_t handle, const std::vector<Descriptor>& ops) {
  return MakeFinalized(CUDNN_BACKEND_OPERATIONGRAPH_DESCRIPTOR,
                       CUDNN_ATTR_OPERATIONGRAPH_HANDLE, handle,
                       CUDNN_ATTR_OPERATIONGRAPH_OPS, ops);
}
...
std::vector<Descriptor> MakeFallbackPlans(
  ...
  for (auto ix : ixs) {
    ...
    auto cfg = MakeFinalized(CUDNN_BACKEND_ENGINECFG_DESCRIPTOR,
                             CUDNN_ATTR_ENGINECFG_ENGINE, engine);
    auto plan = Make(CUDNN_BACKEND_EXECUTION_PLAN_DESCRIPTOR,
                     CUDNN_ATTR_EXECUTION_PLAN_HANDLE, handle,
                     CUDNN_ATTR_EXECUTION_PLAN_ENGINE_CONFIG, cfg);
    ...
    if (workspace > workspace_limit) continue;
...
Storage::Handle AllocWorkspace(std::vector<Descriptor>* plans, size_t* workspace_size) {
  ...
  while ((workspace = FailsafeAlloc(alloc_size)).dptr == nullptr &&
         alloc_size > 0) {
    ...
    plans->erase(std::remove_if(plans->begin(), plans->end(), hasMaxWorkspace),
                plans->end());
...
Descriptor SelectPlan(const OpContext& ctx, const ConvParam& param, Descriptor op,
                      size_t n_fallbacks, const std::function<std::string()>& make_op_str,
                      const std::vector<int64_t>& ids, const std::vector<void*>& tensor_ptrs,
                      int64_t out_size, const std::string& excl_engines_var) {
  ...
  if (verbose > 0) LOG(INFO) << "Selecting plan for " << make_op_str() << ":";
  ...
  if (tune != conv::kOff && param.add_to) {
    ...
    if (out_space.dptr) ptrs.back() = out_space.dptr;
  }
  // Todo:
  ...
  if (tune == conv::kOff || plans.size() == 1 || (param.add_to && !out_space.dptr)) {
    if (verbose > 0) LOG(INFO) << " " << PlanStr(plans[0]);
  ...
  auto var_pack = MakeFinalized(CUDNN_BACKEND_VARIANT_PACK_DESCRIPTOR,
                                ...
                                CUDNN_ATTR_VARIANT_PACK_WORKSPACE, workspace.dptr);
...
enum MaxFallbacks {
  kMaxConvFallbacks = 58,
  kMaxDgradFallbacks = 63,
