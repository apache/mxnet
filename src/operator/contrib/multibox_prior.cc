...
Operator* CreateOp<cpu>(MultiBoxPriorParam param, int dtype) {
  Operator* op = nullptr;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, { op = new MultiBoxPriorOp<cpu, DType>(param); });
  return op;
}

Operator* MultiBoxPriorProp::CreateOperatorEx(Context ctx,
...
NNVM_REGISTER_OP(_contrib_MultiBoxPrior).add_alias("_npx_multibox_prior");
