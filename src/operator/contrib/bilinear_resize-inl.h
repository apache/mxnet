...
struct BilinearSampleParam : public dmlc::Parameter<BilinearSampleParam> {
  ...
  DMLC_DECLARE_PARAMETER(BilinearSampleParam) {
    DMLC_DECLARE_FIELD(height).set_default(1).set_lower_bound(1).describe(
        "output height (required, but ignored if scale_height is defined or mode is not "
        "\"size\")");
    DMLC_DECLARE_FIELD(width).set_default(1).set_lower_bound(1).describe(
        "output width (required, but ignored if scale_width is defined or mode is not "
        "\"size\")");
    DMLC_DECLARE_FIELD(scale_height)
        .set_default(dmlc::optional<float>())
        .describe(
            "sampling scale of the height (optional, used in modes \"scale\" and \"odd_scale\")");
    DMLC_DECLARE_FIELD(scale_width)
        .set_default(dmlc::optional<float>())
        .describe(
    ...
    DMLC_DECLARE_FIELD(mode)
        ...
        .describe(
            "resizing mode. \"simple\" - output height equals parameter \"height\" if "
    ...
    DMLC_DECLARE_FIELD(align_corners)
        .set_default(true)
        .describe(
            "With align_corners = True, the interpolating doesn't proportionally align the"
            "output and input pixels, and thus the output values can depend on the input size.");
  }
};

template <typename DType>
static inline DType area_pixel_compute_scale(int64_t input_size,
                                             int64_t output_size,
                                             bool align_corners) {
...
template <typename DType>
static inline DType area_pixel_compute_source_index(DType scale,
                                                    int64_t dst_index,
                                                    bool align_corners,
                                                    bool cubic) {
  if (align_corners) {
...
inline void BilinearSampleOpBackward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH_EX(inputs[0].type_flag_, DType, AccReal, {
    SpatialUpSamplingBilinearUpdateGradInput<xpu, DType, AccReal>(
        s, inputs, outputs, modeLike, align_corners);
  });
}
