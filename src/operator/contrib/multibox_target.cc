...
inline void AssignLocTargets(const DType* anchor,
                             const DType* l,
                             DType* dst,
                             const float vx,
                             const float vy,
...
inline void MultiBoxTargetForward(const Tensor<cpu, 2, DType>& loc_target,
  ...
  for (int nbatch = 0; nbatch < num_batches; ++nbatch) {
    ...
    if (num_valid_gt > 0) {
      ...
      std::vector<std::pair<float, int>> max_matches(num_anchors, std::pair<float, int>(-1.0f, -1));
      ...
      if (negative_mining_ratio > 0) {
        ...
        if (num_negative > 0) {
          ...
          for (int j = 0; j < num_anchors; ++j) {
            ...
            if (max_matches[j].first < negative_mining_thresh && anchor_flags[j] == -1) {
        ...
        if (anchor_flags[i] == 1) {
          ...
          *(p_loc_mask + offset + 3) = 1;
          AssignLocTargets(p_anchor + i * 4,
                           p_label + label_width * max_matches[i].second + 1,
                           p_loc_target + offset,
                           variances[0],
                           variances[1],
                           variances[2],
...
Operator* CreateOp<cpu>(MultiBoxTargetParam param, int dtype) {
  Operator* op = nullptr;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, { op = new MultiBoxTargetOp<cpu, DType>(param); });
  return op;
}

Operator* MultiBoxTargetProp::CreateOperatorEx(Context ctx,
...
NNVM_REGISTER_OP(_contrib_MultiBoxTarget).add_alias("_npx_multibox_target");
