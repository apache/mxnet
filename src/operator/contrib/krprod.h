...
inline void row_wise_kronecker(Tensor<cpu, 2, DType> out,
  ...
  Tensor<cpu, 2, DType>*given = &storage, *result = &out, *tmp;

  // Compute each intermediate row-wise Kronecker product
  storage = 1;
  ncols   = 1;
  for (auto& ts : ts_arr) {
    expr::BLASEngine<cpu, DType>::SetStream(result->stream_);
    ...
    *result = 0;
    for (int i = 0; i < nrows; ++i) {
      ...
      // )
      expr::BLASEngine<cpu, DType>::ger(result->stream_,
                                        ts.size(1),
                                        ncols,
                                        1,
                                        ts[i].dptr_,
 ...
 */
template <typename DType>
inline void khatri_rao(Tensor<cpu, 2, DType> out,
                       const std::vector<Tensor<cpu, 2, DType> >& ts_arr) {
  CHECK_GE(ts_arr.size(), 1) << "The input matrices must be non-empty.";
  ...
  AllocSpace(&out_t);
  flip<cpu, DType>(out.size(0), out.size(1), out_t.dptr_, out_t.stride_, out.dptr_, out.stride_);
  ...
  for (int i = 0; i < static_cast<int>(ts_arr.size()); ++i) {
    ...
    flip<cpu, DType>(ts_arr[i].size(0),
                     ts_arr[i].size(1),
                     ts_t_arr[i].dptr_,
                     ts_t_arr[i].stride_,
                     ts_arr[i].dptr_,
  ...
  flip<cpu, DType>(out.size(1), out.size(0), out.dptr_, out.stride_, out_t.dptr_, out_t.stride_);
 ...
 *
 * Given input matrices A_1, ..., A_n, of shape (l_1, k), ..., (l_n, k) respectively, the
 * pseudoinverse of the Khatri-Rao product is
 *
 ...
 *
 * As the first term of the r.h.s is a square matrix, the result is always of the same shape as the
 * transpose of the Khatri-Rao product of the input matrices. The input argument ts_arr could
 * contain the original input matrices, or transposed ones.
 *
 * \param out result matrix
 * \param ts_arr vector of input matrices
 ...
 */
template <typename DType>
inline void inv_khatri_rao(Tensor<cpu, 2, DType> out,
                           const std::vector<Tensor<cpu, 2, DType> >& ts_arr,
  ...
  if (input_transposed) {
    ...
    info = MXNET_LAPACK_posv<DType>(MXNET_LAPACK_ROW_MAJOR,
                                    'U',
                                    k,
                                    out.size(1),
                                    hadamard_prod.dptr_,
                                    hadamard_prod.stride_,
                                    out.dptr_,
                                    out.stride_);
  } else {
    ...
    info = MXNET_LAPACK_posv<DType>(MXNET_LAPACK_COL_MAJOR,
                                    'U',
                                    k,
                                    out.size(1),
                                    hadamard_prod.dptr_,
    ...
    flip<cpu, DType>(out.size(1), out.size(0), out.dptr_, out.stride_, kr.dptr_, kr.stride_);
    FreeSpace(&kr);
  }
...
inline void KhatriRaoCompute_(const nnvm::NodeAttrs& attrs,
  ...
  std::transform(
      ...
      in_data.end(),
      ts_arr.begin(),
      [&stream](TBlob blob) -> Tensor<xpu, 2, DType> { return blob.get<xpu, 2, DType>(stream); });
