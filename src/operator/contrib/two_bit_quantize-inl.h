...
struct quantize_2bit {
  MSHADOW_XINLINE static void Map(int i,
    ...
    int col_id = (i%16)%4;
    // Compress
    if (*(in+i) <= *neg_threshold) {  // set data to 01
      switch (col_id) {
        case 0:
    ...
    } else if (*(in+i) >= *pos_threshold) {  // set data to 10
      switch (col_id) {
        case 0:
...
void Quantize2BitCompute(const nnvm::NodeAttrs& attrs,
  ...
  Kernel<quantize_2bit, xpu>::Launch(s, inputs[0].Size(),
                          ...
                          inputs[1].dptr<float>(),     // negative threshold
                          inputs[2].dptr<float>());    // positive threshold
}
...
inline bool Quantize2BitType(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_EQ((*in_attrs)[1], mshadow::kFloat32)
    << "the second input of `quantize_2bit` should be "
