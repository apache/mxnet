...
*/
#ifndef MXNET_OPERATOR_CONTRIB_ROI_ALIGN_MAX_INL_H_
#define MXNET_OPERATOR_CONTRIB_ROI_ALIGN_MAX_INL_H_
...
namespace roialign {
enum ROIAlignMaxOpInputs {kData, kBox};
enum ROIAlignMaxOpOutputs {kOut, kMaxIdx_x, kMaxIdx_y};
}  // roialign


struct ROIAlignMaxParam : public dmlc::Parameter<ROIAlignMaxParam> {
  TShape pooled_size;
  float spatial_scale;
  DMLC_DECLARE_PARAMETER(ROIAlignMaxParam) {
...
/*!
 * \brief Kernel for forward pass of ROIAlignMax.
 */
struct ROIAlignMaxForwardKernel {
  /*!
...
void ROIAlignMaxForward(const nnvm::NodeAttrs& attrs,
                        const OpContext& ctx,
  ...
  const ROIAlignMaxParam param = nnvm::get<ROIAlignMaxParam>(attrs.parsed);
  ...
  MSHADOW_REAL_TYPE_SWITCH(in_data[0].type_flag_, DType, {
    ...
    mxnet_op::Kernel<ROIAlignMaxForwardKernel, xpu>::Launch(s,
...
/*!
 * \brief Kernel for backward pass of ROIAlignMax.
 */
struct ROIAlignMaxBackwardKernel {
  /*!
   * \param index          loop index
...
template<typename xpu>
void ROIAlignMaxBackward(const nnvm::NodeAttrs& attrs,
                         const OpContext& ctx,
  ...
  // the order here relates to the order in ROIAlignMaxGrad
  ...
  CHECK_NE(req[0], kWriteInplace) <<
    "ROIAlignMax: Backward doesn't support kWriteInplace.";
  ...
  MSHADOW_REAL_TYPE_SWITCH(out_grad[0].type_flag_, DType, {
    ...
    if (kAddTo == req[roialign::kData] || kWriteTo == req[roialign::kData]) {
      ...
      mxnet_op::Kernel<ROIAlignMaxBackwardKernel, xpu>::Launch(s,
...
#endif  // MXNET_OPERATOR_CONTRIB_ROI_ALIGN_MAX_INL_H_
