  ...
  virtual void Forward(const OpContext &ctx,
    ...
    //   .get_space_typed<xpu, 1, DType>(cpu_bias.shape_, s);
     ...
     if (param_.nms_threshold > 0 && param_.nms_threshold < 1) {
       int keep = param_.nms_topk < 0 ? out.shape_[1] : 0;
       ...
       if (keep > 0) {
         ...
         Shape<3> out_shape = Shape3(num_batch, num_elem, out.shape_[2]);
         index_t nms_ws_size = 3 * sort_index_shape.Size() + out_shape.Size();
         ...
         Tensor<xpu, 1, DType> batch_id(scores.dptr_ + scores.MSize(),
          sort_index_shape, s);
         Tensor<xpu, 3, DType> out_copy(batch_id.dptr_ + batch_id.MSize(),
          out_shape, s);

         // copy score to buffer, sort accordingly to get sorted index
         scores = reshape(slice<1>(out, 1, 2), scores.shape_);
         ...
         out_copy = -1;
         Kernel<nms_assign, xpu>::Launch(s, num_batch, out_copy.dptr_,
          out.dptr_, sorted_index.dptr_, keep, num_elem, out.shape_[2]);
         // copy back to output
         out = out_copy;
       }
     }
