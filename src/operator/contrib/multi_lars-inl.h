...
struct LARSParam : public dmlc::Parameter<LARSParam> {
  ...
  DMLC_DECLARE_PARAMETER(LARSParam) {
    DMLC_DECLARE_FIELD(eta).describe("LARS eta");
    DMLC_DECLARE_FIELD(eps).describe("LARS eps");
...
struct MultiLARSKernel {
  MSHADOW_XINLINE static void Map(index_t i,
                                  float* out_data,
                                  const float* lrs,
                                  const float* weights_sum_sq,
                                  const float* grads_sum_sq,
                                  const float* wds,
                                  const float eta,
                                  const float eps,
...
inline void MultiLARS(const nnvm::NodeAttrs& attrs,
  ...
  Kernel<MultiLARSKernel, xpu>::Launch(s,
                                       weights_sum_sq.shape_.Size(),
                                       out_data.dptr_,
                                       lrs.dptr_,
                                       weights_sum_sq.dptr_,
                                       grads_sum_sq.dptr_,
                                       wds.dptr_,
                                       param.eta,
                                       param.eps,
