...
DMLC_REGISTER_PARAMETER(ROIAlignMaxParam);
...
RoI bin, and aggregate the result using max pooling.
...
.set_num_inputs(2)
.set_num_outputs(3)
.set_attr_parser(ParamParser<ROIAlignMaxParam>)
...
.set_attr<FCompute>("FCompute<cpu>", ROIAlignMaxForward<cpu>)
.set_attr<nnvm::FGradient>("FGradient", ROIAlignMaxGrad{"_backward_ROIAlignMax"})
...
.add_arguments(ROIAlignMaxParam::__FIELDS__());
...
.set_attr_parser(ParamParser<ROIAlignMaxParam>)
.set_attr<FCompute>("FCompute<cpu>", ROIAlignMaxBackward<cpu>);
