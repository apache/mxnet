 ...
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    if (param_.op_type == channel_operator::kGroupSoftmax) {
      ...
      const Shape<3> data_shape =
        Shape3(batch_size*param_.group, channel_num / param_.group, rest_size);

      Tensor<xpu, 3, DType> data =
        in_data[channel_operator::kData].get_with_shape<xpu, 3, DType>(data_shape, s);
      Tensor<xpu, 3, DType> out =
        out_data[channel_operator::kOut].get_with_shape<xpu, 3, DType>(data_shape, s);
      Softmax(out, data);
      ...
      if (param_.pick_type == channel_operator::kScore) {
        Tensor<xpu, 4, DType> argmax_data =
          out_data[channel_operator::kTemp].get<xpu, 4, DType>(s);
