...
struct CountSketchParam : public dmlc::Parameter<CountSketchParam> {
  ...
  DMLC_DECLARE_PARAMETER(CountSketchParam) {
    DMLC_DECLARE_FIELD(out_dim).describe("The output dimension.");
    DMLC_DECLARE_FIELD(processing_batch_size)
        .set_default(32)
...
class CountSketchOp : public Operator {
 public:
  ...
  virtual void Forward(const OpContext& ctx,
    ...
    out = 0;
    CountSketchForward(out,
                       data,
                       h,
                       ss,
                       n_samples,
                       this->param_.processing_batch_size,
                       in_dim,
                       this->param_.out_dim);
  }
...
class CountSketchProp : public OperatorProperty {
 public:
  ...
  std::vector<int> DeclareBackwardDependency(const std::vector<int>& out_grad,
    ...
    return {out_grad[CountSketch::kOut],
            in_data[CountSketch::kData],
            in_data[CountSketch::kH],
            in_data[CountSketch::kS]};
  }
