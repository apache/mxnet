...
struct GroupAdagradParam : public dmlc::Parameter<GroupAdagradParam> {
  ...
  DMLC_DECLARE_PARAMETER(GroupAdagradParam) {
    ...
    DMLC_DECLARE_FIELD(epsilon).set_default(1.0e-5).describe("Epsilon for numerical stability");
  }
};
...
struct GroupAdagradDnsRspKernel {
  ...
  MSHADOW_XINLINE static void Map(int i,
    ...
    auto get_grad_rescaled =
        [&i, &row_length, &grad_data, &rescale_grad, &clip_gradient](index_t j) -> DType {
 ...
 */
template <typename xpu>
inline void GroupAdagradUpdateDnsRspDnsImpl(const GroupAdagradParam& param,
                                            const OpContext& ctx,
                                            const TBlob& weight,
                                            const NDArray& grad,
                                            const TBlob& state,
  ...
  CHECK_EQ(req, kWriteInplace) << "kWriteInplace is expected for sparse group_adagrad_update";
  ...
  MSHADOW_REAL_TYPE_SWITCH(weight.type_flag_, DType, {
    MSHADOW_IDX_TYPE_SWITCH(grad.aux_type(rowsparse::kIdx), IType, {
      ...
      Kernel<GroupAdagradDnsRspKernel<xpu>, xpu>::Launch(s,
                                                         num_grad,
                                                         row_length,
                                                         out_data,
                                                         state_data,
 ...
 */
template <typename xpu>
inline void GroupAdagradUpdateRspRspRspImpl(const GroupAdagradParam& param,
                                            const OpContext& ctx,
  ...
  GroupAdagradUpdateDnsRspDnsImpl<xpu>(
      param, ctx, weight.data(), grad, state.data(), req, &out_blob);
}
...
inline void GroupAdagradUpdateEx(const nnvm::NodeAttrs& attrs,
  ...
  if (state_stype == weight_stype && output_stype == weight_stype &&
    ...
    GroupAdagradUpdateRspRspRspImpl<xpu>(param, ctx, inputs[0], inputs[1], inputs[2], req[0], &out);
