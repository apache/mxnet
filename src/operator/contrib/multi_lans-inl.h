...
inline void MultiLANS(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    ...
    ctx.requested[multilans::kTempSpace].get_space_typed<xpu, 1, char>(
    ...
    Tensor<xpu, 1, float> r2_m(reinterpret_cast<float*>(&workspace[pos_wspace]),
      Shape1(kernel_params.ntensors), s);
    pos_wspace += kernel_params.ntensors * sizeof(float);
    Tensor<xpu, 1, float> r2_g(reinterpret_cast<float*>(&workspace[pos_wspace]),
      Shape1(kernel_params.ntensors), s);
    pos_wspace += kernel_params.ntensors * sizeof(float);
    Tensor<xpu, 1, float> g_sq_norm(reinterpret_cast<float*>(&workspace[pos_wspace]),
         Shape1(kernel_params.ntensors), s);
    pos_wspace += kernel_params.ntensors * sizeof(float);
    ...
    CallKernel2<MPDType, DType>(s, kernel_params, param, r1.dptr_,
                                r2_m.dptr_, r2_g.dptr_,
                                temp_m.dptr_, temp_g.dptr_,
...
inline void MultiLANSUpdate(const nnvm::NodeAttrs& attrs,
                               ...
                               const std::vector<TBlob> &outputs) {
  if (!MP) {
    MultiLANS<xpu, LANSTypeIdentity, 4>
      (attrs, ctx, inputs, req, outputs);
  } else {
    MultiLANS<xpu, LANSSinglePrecision, 5>
      (attrs, ctx, inputs, req, outputs);
  }
}
