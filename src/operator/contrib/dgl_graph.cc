...
struct NeighborSampleParam : public dmlc::Parameter<NeighborSampleParam> {
  ...
  DMLC_DECLARE_PARAMETER(NeighborSampleParam) {
    DMLC_DECLARE_FIELD(num_args).set_lower_bound(2).describe("Number of input NDArray.");
    DMLC_DECLARE_FIELD(num_hops).set_default(1).describe("Number of hops.");
    DMLC_DECLARE_FIELD(num_neighbor).set_default(2).describe("Number of neighbor.");
    DMLC_DECLARE_FIELD(max_num_vertices).set_default(100).describe("Max number of vertices.");
  }
};
...
static void RandomSample(size_t set_size, size_t num, std::vector<size_t>* out, unsigned int seed) {
...
static void NegateSet(const std::vector<size_t>& idxs, size_t set_size, std::vector<size_t>* out) {
...
struct neigh_list {
  ...
  neigh_list(std::vector<dgl_id_t> _neighs, std::vector<dgl_id_t> _edges)
...
static void SampleSubgraph(const NDArray& csr,
  ...
  size_t idx = 0;
  while (idx < sub_vers.size() && sub_ver_mp.size() < max_num_vertices) {
  ...
  for (size_t i = 0; i < num_vertices; i++) {
    ...
    if (idx_with_neigh < neigh_pos.size() && dst_id == neigh_pos[idx_with_neigh].first) {
      ...
      std::copy_n(neighbor_list.begin() + pos + 1, edge_size, col_list_out + collected_nedges);
      std::copy_n(
          neighbor_list.begin() + pos + edge_size + 1, edge_size, val_list_out + collected_nedges);
...
struct DGLSubgraphParam : public dmlc::Parameter<DGLSubgraphParam> {
  ...
  DMLC_DECLARE_PARAMETER(DGLSubgraphParam) {
    DMLC_DECLARE_FIELD(num_args).set_lower_bound(2).describe(
        "Number of input arguments, including all symbol inputs.");
...
class HashTableChecker {
 ...
 public:
  ...
  void CollectOnRow(const dgl_id_t col_idx[],
                    const dgl_id_t eids[],
                    size_t row_len,
  ...
  void Collect(const dgl_id_t old_id,
               const dgl_id_t old_eid,
...
static void GetSubgraph(const NDArray& csr_arr,
                        const NDArray& varr,
                        const NDArray& sub_csr,
                        const NDArray* old_eids) {
  ...
  for (size_t i = 0; i < len; ++i) {
    ...
    CHECK_LT(oldvid, num_vertices)
        << "Vertex Id " << oldvid << " isn't in a graph of " << num_vertices << " vertices";
    size_t row_start = indptr[oldvid];
    size_t row_len   = indptr[oldvid + 1] - indptr[oldvid];
    def_check.CollectOnRow(indices + row_start,
                           eids + row_start,
                           row_len,
                           &col_idx,
                           old_eids == nullptr ? nullptr : &orig_eids);
...
Example:
   ...
   .. code:: python
    ...
    .set_attr<nnvm::FListInputNames>("FListInputNames",
                                     [](const NodeAttrs& attrs) {
                                       const DGLSubgraphParam& params =
                                           nnvm::get<DGLSubgraphParam>(attrs.parsed);
...
struct edge_id_csr_forward {
  ...
  MSHADOW_XINLINE static void Map(int i,
    ...
    auto ptr                    = std::find(in_indices + in_indptr[target_row_id],
                         in_indices + in_indptr[target_row_id + 1],
                         target_col_id);
  ...
  MSHADOW_TYPE_SWITCH(data.dtype(), DType, {
    MSHADOW_IDX_TYPE_SWITCH(data.aux_type(kIdx), IType, {
      MSHADOW_TYPE_SWITCH(u.dtype(), CType, {
        Kernel<edge_id_csr_forward, xpu>::Launch(s,
                                                 out_elems,
                                                 output.data().dptr<DType>(),
                                                 in_data.dptr<DType>(),
                                                 in_indices.dptr<IType>(),
                                                 in_indptr.dptr<IType>(),
                                                 u.data().dptr<CType>(),
                                                 v.data().dptr<CType>());
      });
    });
...
struct SubgraphCompactParam : public dmlc::Parameter<SubgraphCompactParam> {
  ...
  DMLC_DECLARE_PARAMETER(SubgraphCompactParam) {
    ...
    DMLC_DECLARE_FIELD(return_mapping)
        .describe("Return mapping of vid and eid between the subgraph and the parent graph.");
    DMLC_DECLARE_FIELD(graph_sizes).describe("the number of vertices in each graph.");
  }
...
static void CompactSubgraph(const NDArray& csr,
                            const NDArray& vids,
...
Example:
  ...
  array([[0, 0, 0, 1, 0],
    ...
    .set_attr<nnvm::FListInputNames>("FListInputNames",
                                     [](const NodeAttrs& attrs) {
                                       const SubgraphCompactParam& params =
                                           nnvm::get<SubgraphCompactParam>(attrs.parsed);
                                       std::vector<std::string> names;
                                       names.reserve(params.num_args);
