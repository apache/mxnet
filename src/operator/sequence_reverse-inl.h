...
class SequenceReverseOp : public Operator {
 public:
  ...
  void sequence_reverse(
                        const mshadow::Tensor<xpu, 3, DType> data,
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    if (param_.use_sequence_length)
      index_array_copy(in_data[seq_reverse::kSequenceLength], &indices_vec, ctx);
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    sequence_reverse(output_grad, data_grad, indices_vec, req[seq_reverse::kData]);
  }
