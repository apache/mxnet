...
class ConvolutionOp {
 public:
  ...
  void Forward(const OpContext& ctx,
    ...
    if (is_1x1_) {
      Tensor<xpu, 4, DType> input_4d =
          in_data[conv::kData].get_with_shape<xpu, 4, DType>(Shape4(num_, group_, K, N), s);
      ...
      Tensor<xpu, 1, DType> workspace =
          ctx.requested[conv::kTempSpace].get_space_typed<xpu, 1, DType>(Shape1(col_buffer_size_),
                                                                         s);
  ...
  void Backward(const OpContext& ctx,
    ...
    _BackwardWeightsBias(ctx,
                         out_grad[conv::kOut],
                         in_data[conv::kData],
                         req[conv::kWeight],
                         in_grad[conv::kWeight],
                         bias_term_ ? req[conv::kBias] : OpReqType(),
                         bias_term_ ? &in_grad[conv::kBias] : nullptr);
  }

 private:
  // Computes dLoss/dData
  void _BackwardData(const OpContext& ctx,
  ...
  void _BackwardWeightsBias(const OpContext& ctx,
                            const TBlob& out_grad,
                            const TBlob& data,
    ...
    // bias gradient
    if (bias_grad_dst != nullptr) {
      CHECK_EQ(param_.no_bias, false);
