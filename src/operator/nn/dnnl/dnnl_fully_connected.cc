...
void DNNLFCForwardFullFeature(const DNNLFCFullParam& full_param,
  ...
  if (!full_param.default_param.no_bias) {
    ...
    auto bias_mem =
        static_cast<const dnnl::memory*>(in_data[fullc::kBias].GetDNNLDataReorder(&fwd_bias_desc));
...
void DNNLFCBackward(const nnvm::NodeAttrs& attrs,
  ...
  if (req[fullc::kWeight]) {
    ...
    auto out_grad_mem =
        static_cast<const dnnl::memory*>(out_grad.GetDNNLDataReorder(&ipBwdWeights_diff_dst_desc));
    auto data_mem =
        static_cast<const dnnl::memory*>(data.GetDNNLDataReorder(&ipBwdWeights_src_desc));
  ...
  if (req[fullc::kData]) {
    ...
    auto out_grad_mem =
        static_cast<const dnnl::memory*>(out_grad.GetDNNLDataReorder(&ipBwdData_diff_dst_desc));
    auto weight_mem =
        static_cast<const dnnl::memory*>(weight.GetDNNLDataReorder(&ipBwdData_weight_desc));
    auto in_grad_mem =
