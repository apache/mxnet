...
void DNNLConvolutionForwardFullFeature(const DNNLConvFullParam& param,
  ...
  if (ctx.is_train) {
    ...
    if (weight.IsDefaultData()) {
      ...
      auto fwd_weight_desc = fwd->GetPd().weights_desc();
      weight_mem = static_cast<const dnnl::memory*>(weight.GetDNNLDataReorder(&fwd_weight_desc));
    }
  }
  dnnl_output_t out_mem;
  if (param.dnnl_param.with_sum) {
    out_mem = dnnl_output_t(OutDataOp::Noop,
                            const_cast<dnnl::memory*>(static_cast<const dnnl::memory*>(
                                out_data[conv::kOut].GetDNNLData())));
  } else {
  ...
  if (!no_bias) {
    const dnnl::memory* bias_mem =
        static_cast<const dnnl::memory*>(in_data[conv::kBias].GetDNNLData());
...
void DNNLConvolutionBackward(const nnvm::NodeAttrs& attrs,
  ...
  auto out_grad_mem =
      static_cast<const dnnl::memory*>(out_grad.GetDNNLDataReorder(&convBwd_data_diff_desc));
  if (req[conv::kData]) {
  ...
  if (req_weight || req_bias) {
    if (convBwd.GetDataPd().diff_dst_desc() != convBwd.GetWeightsPd().diff_dst_desc()) {
      ...
      out_grad_mem =
          static_cast<const dnnl::memory*>(out_grad.GetDNNLDataReorder(&convBwd_weight_diff_desc));
    }
    auto convBwd_weight_src_desc = convBwd.GetWeightsPd().src_desc();
    auto data_mem =
        static_cast<const dnnl::memory*>(data.GetDNNLDataReorder(&convBwd_weight_src_desc));
