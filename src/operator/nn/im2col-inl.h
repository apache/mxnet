...
struct Im2colParam : public dmlc::Parameter<Im2colParam> {
  ...
  DMLC_DECLARE_PARAMETER(Im2colParam) {
    ...
    DMLC_DECLARE_FIELD(stride)
        ...
        .describe(
            "The stride between adjacent sliding blocks in spatial dimension: "
            "(w,), (h, w) or (d, h, w). Defaults to 1 for each dimension.");
    DMLC_DECLARE_FIELD(dilate)
        .set_default(mxnet::TShape(0, 0))
        .describe(
            "The spacing between adjacent kernel points: (w,), (h, w) or (d, h, w). "
            "Defaults to 1 for each dimension.");
    DMLC_DECLARE_FIELD(pad)
        .set_default(mxnet::TShape(0, 0))
        .describe(
            "The zero-value padding size on both sides of spatial dimension: "
...
void Im2colCompute(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    ...
    if (req[0] != kAddTo) {
      ...
      Tensor<xpu, 2, DType> tcol =
          ctx.requested[0].get_space_typed<xpu, 2, DType>(Shape2(col_shape[1], col_shape[2]), s);
      for (index_t n = 0; n < num; ++n) {
        im2col(s,
               im[n].dptr_,
               im_shape,
