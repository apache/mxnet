...
inline static mkldnn::inner_product_forward::primitive_desc GetIPFwd(
  ...
  if (bias) {
    ...
    mkldnn::inner_product_forward::desc ipFwd_desc(propagation, data_md, weight_md, bias_md,
                                                   out_md);
    return mkldnn::inner_product_forward::primitive_desc(ipFwd_desc, engine);
  } else {
    mkldnn::inner_product_forward::desc ipFwd_desc(propagation, data_md, weight_md, out_md);
    return mkldnn::inner_product_forward::primitive_desc(ipFwd_desc, engine);
  }
}
...
inline static mkldnn::inner_product_backward_weights::primitive_desc GetIPBwdWeights(
  ...
  if (bias) {
    auto bias_md = GetMemDesc(*bias);
    mkldnn::inner_product_backward_weights::desc ipBwdWeights_desc(data_md, weight_md, bias_md,
                                                                   out_md);
    return mkldnn::inner_product_backward_weights::primitive_desc(ipBwdWeights_desc, engine,
                                                                  ipFwd_pd);
  } else {
    mkldnn::inner_product_backward_weights::desc ipBwdWeights_desc(data_md, weight_md, out_md);
    return mkldnn::inner_product_backward_weights::primitive_desc(ipBwdWeights_desc, engine,
                                                                  ipFwd_pd);
  }
}
...
void MKLDNNFCForward(const nnvm::NodeAttrs &attrs, const OpContext &ctx,
  ...
  if (data.IsMKLDNNData() && data.IsView()) data = in_data[fullc::kData].Reorder2Default();
  ...
  mkldnn::inner_product_forward::primitive_desc ipFwd_pd = GetIPFwd(
      data, weight, param.no_bias ? nullptr : &in_data[fullc::kBias], out_md, ctx.is_train);
  ...
  auto out_mem =
      CreateMKLDNNMem(out_data[fullc::kOut], ipFwd_pd.dst_primitive_desc(), req[fullc::kOut]);
  if (param.no_bias) {
    MKLDNNStream::Get()->RegisterPrim(
        mkldnn::inner_product_forward(ipFwd_pd, *data_mem, *weight_mem, *out_mem.second));
  } else {
    ...
    MKLDNNStream::Get()->RegisterPrim(mkldnn::inner_product_forward(
        ipFwd_pd, *data_mem, *weight_mem, *bias_mem, *out_mem.second));
  }
...
void MKLDNNFCBackward(const nnvm::NodeAttrs &attrs, const OpContext &ctx,
  ...
  mkldnn::inner_product_forward::primitive_desc ipFwd_pd =
      GetIPFwd(data, weight, param.no_bias ? nullptr : &in_grad[fullc::kBias], GetMemDesc(out_grad),
               ctx.is_train);

  CHECK_NE(req[fullc::kWeight], kWriteInplace) << "cannot write weight inplace";
  if (req[fullc::kData]) {
    mkldnn::inner_product_backward_data::primitive_desc ipBwdData_pd =
        GetIpBwdData(data, weight, out_grad, ipFwd_pd);
    auto out_grad_mem = out_grad.GetMKLDNNDataReorder(ipBwdData_pd.diff_dst_primitive_desc());
  ...
  if (req[fullc::kWeight]) {
    mkldnn::inner_product_backward_weights::primitive_desc ipBwdWeights_pd = GetIPBwdWeights(
        data, weight, param.no_bias ? nullptr : &in_grad[fullc::kBias], out_grad, ipFwd_pd);
    auto out_grad_mem = out_grad.GetMKLDNNDataReorder(ipBwdWeights_pd.diff_dst_primitive_desc());
    auto data_mem = data.GetMKLDNNDataReorder(ipBwdWeights_pd.src_primitive_desc());
    auto in_grad_weight =
        CreateMKLDNNWeightGrad(in_grad[fullc::kWeight],
    ...
    if (param.no_bias) {
      ...
      MKLDNNStream::Get()->RegisterPrim(mkldnn::inner_product_backward_weights(
          ipBwdWeights_pd, *data_mem, *out_grad_mem, *in_grad_weight.second, *in_grad_bias.second));
    }
