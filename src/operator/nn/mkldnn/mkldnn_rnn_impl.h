...
static void AdjustGruWeightGateOrder(DType* weight,
  ...
  for (int i = 0; i < static_cast<int>(single_weight_size); i++) {
...
static void MKLDNNRNNForwardSingleLayerBi(bool state_outputs,
  ...
  if (!initialized || is_train) {
    ...
    if (mode == rnn_enum::kGru) {
      ...
      #pragma omp parallel for num_threads(omp_threads)
      for (int j = 0; j < static_cast<int>(mx_single_b_sz); j++) {
  ...
  if (state_outputs) {
    ...
    if (mode == rnn_enum::kLstm) {
      ...
      #pragma omp parallel for num_threads(omp_threads)
      for (int n = 0; n < static_cast<int>(single_cell_size); n++) {
        hy_ptr[n] = dst_hcy[n];
      ...
      #pragma omp parallel for num_threads(omp_threads)
      for (int n = 0; n < static_cast<int>(2 * single_cell_size); n++) {
        hy_ptr[n] = dst_hcy[n];
      }
    }
  }
...
static void MKLDNNRNNForwardUnidi(const bool state_outputs,
  ...
  if (!initialized || is_train) {
    ...
    if (mode == rnn_enum::kGru) {
      ...
      #pragma omp parallel for num_threads(omp_threads)
      for (int j = 0; j < static_cast<int>(b_size); j++) {
  ...
  if (state_outputs) {
    ...
    if (mode == rnn_enum::kLstm) {
      ...
      #pragma omp parallel for num_threads(omp_threads)
      for (int n = 0; n < static_cast<int>(cell_size); n++) {
        hy_ptr[n] = dst_hcy[n];
      }
    }
  }
