...
MKLDNNPoolingFwd &GetPoolingFwd(const PoolingParam &param,
  ...
  if (it == pooling_fwds.end()) {
    ...
    auto ins_ret = AddToCache(pooling_fwds, std::pair<MKLDNNPoolingSignature, MKLDNNPoolingFwd>(key, fwd));
...
MKLDNNPoolingBwd &GetPoolingBwd(const PoolingParam &param,
  ...
  if (it == pooling_bwds.end()) {
    ...
    auto ins_ret = AddToCache(pooling_bwds, std::pair<MKLDNNPoolingSignature, MKLDNNPoolingBwd>(key, bwd));
    CHECK(ins_ret.second);
    it = ins_ret.first;
  }
  return it->second;
