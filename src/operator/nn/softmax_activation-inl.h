...
struct SoftmaxActivationParam : public dmlc::Parameter<SoftmaxActivationParam> {
  ...
  DMLC_DECLARE_PARAMETER(SoftmaxActivationParam) {
    DMLC_DECLARE_FIELD(mode)
        ...
        .describe(
            "Specifies how to compute the softmax. If set to ``instance``, "
...
void SoftmaxActivationGradCompute(const nnvm::NodeAttrs& attrs,
  ...
  // Get tensors
  Stream<xpu>* s            = ctx.get_stream<xpu>();
  Tensor<xpu, 3> m_out_grad = out_grad.get_with_shape<xpu, 3, real_t>(data_shape, s);
  Tensor<xpu, 3> m_out_data = out_data.get_with_shape<xpu, 3, real_t>(data_shape, s);
  Tensor<xpu, 3> m_in_grad  = in_grad.get_with_shape<xpu, 3, real_t>(data_shape, s);
  ...
  Assign(
      m_in_grad, req, m_out_data * (m_out_grad - broadcast_with_axis(workspace, 0, channel_num)));
}
