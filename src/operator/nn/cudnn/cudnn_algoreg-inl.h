      ...
      if (param.cudnn_tune.value() && reg_.size() % 50 == 0) {
        ...
        if (reg_.size() >= 1000) {
          ...
          if (!is_warning_autotune_) {
            LOG(INFO) << "If you see this message in the middle of training, you are "
 ...
 private:
  ...
  struct ParamKey {
    ...
    bool operator==(const ParamKey& other) const {
      return this->param == other.param && this->data_shape == other.data_shape &&
             this->weight_shape == other.weight_shape && this->out_shape == other.out_shape &&
             ...
             this->sm_arch == other.sm_arch && this->add_to_weight == other.add_to_weight;
    }
  };
