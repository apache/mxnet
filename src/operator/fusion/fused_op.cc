...
FusedOp::FusedOp(const nnvm::NodeAttrs* attrs, const FusedOpConfig& config)
    : initialized_(false), kernel_function_dev_id_(-1) {
...
bool FusedOp::InferShape(const nnvm::NodeAttrs& attrs,
  ...
  subgraph_ = mxnet::exec::InferShape(std::move(subgraph_), std::move(input_shapes), "__shape__");
...
bool FusedOp::InferType(const nnvm::NodeAttrs& attrs,
  ...
  subgraph_ = mxnet::exec::InferType(std::move(subgraph_), std::move(input_types), "__dtype__");
...
std::tuple<const nnvm::ObjectPtr, std::vector<Attr>, std::vector<Attr>> FusedOp::GetAttrs(
  ...
  return std::make_tuple(node.weak_ref.lock(), inputs, outputs);
}
...
std::tuple<const nnvm::ObjectPtr, std::vector<int>, std::vector<int>> FusedOpHelperType(
    const NodeAttrs& attrs) {
  const auto& p       = nnvm::get<FusedOpHelperParamPtr>(attrs.parsed);
  const auto& op      = p->op;
  const auto& node_id = p->node_id;
...
std::tuple<const nnvm::ObjectPtr, std::vector<int>, std::vector<int>> FusedOpOutHelperType(
    const NodeAttrs& attrs) {
  const auto& p       = nnvm::get<FusedOpHelperParamPtr>(attrs.parsed);
  const auto& op      = p->op;
  const auto& node_id = p->node_id;
