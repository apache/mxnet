...
NDArray::NDArray(const NDArrayStorageType stype, const mxnet::TShape &shape, Context ctx,
  ...
  dtype_(dtype), storage_type_(stype), autograd_(nullptr) {
...
NDArray NDArray::grad() const {
  ...
  AGInfo& info = AGInfo::Get(autograd_.node);
...
nnvm::Symbol NDArray::get_autograd_symbol() const {
  ...
  nnvm::Symbol ret;
  ret.outputs.emplace_back(autograd_);
  return ret;
}
...
NDArray::NDArray(mkldnn::memory::primitive_desc mem_pd)
    : storage_type_(kDefaultStorage), autograd_(nullptr) {
