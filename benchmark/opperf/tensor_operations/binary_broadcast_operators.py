...
"""Performance benchmark tests for MXNet NDArray Binary Broadcast Operations.
...
Below 20 Operators are covered:
...
from benchmark.opperf.utils.op_registry_utils import get_all_broadcast_binary_operators, prepare_op_inputs
...
def run_mx_binary_broadcast_operators_benchmarks(ctx=mx.cpu(), dtype='float32', warmup=10, runs=50):
    ...
    mx_binary_ops = get_all_broadcast_binary_operators()

    # For each operator, run benchmarks
    mx_binary_op_results = []
    for _, op_params in mx_binary_ops.items():
