...
class HingeLoss(EvalMetric):
    ...
    Examples
    --------
    >>> predicts = [mx.nd.array(np.array([-1.73, -1.24, 0.89, -0.99, 0.05]).reshape(1, -1))]
    ...
    ('hinge_loss', 0.02560000866651535)
    """
    ...
    def update(self, labels, preds):
        ...
        for label, pred in zip(labels, preds):
            ...
            pred = pred - pred[numpy.arange(n), label].reshape(-1, 1) + self.margin
