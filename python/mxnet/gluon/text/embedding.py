    ...
    if embedding_name is not None:
        if embedding_name not in text_embedding_reg:
            raise KeyError('Cannot find `embedding_name` %s. Use '
                           '`get_file_names(embedding_name=None).keys()` to get all the valid'
...
class TokenEmbedding(object):
    ...
    def _load_embedding(self, pretrained_file_path, elem_delim, init_unknown_vec, encoding='utf8'):
        ...
        with io.open(pretrained_file_path, 'r', encoding=encoding) as f:
            for line in f:
                ...
                assert len(elems) > 1, 'At line %d of the pre-trained token embedding file: the ' \
                ...
                if token == self.unknown_token and loaded_unknown_vec is None:
                    ...
                    warnings.warn('At line %d of the pre-trained token embedding file: token %s '
    ...
    def __getitem__(self, tokens):
        ...
        indices = [self._token_to_idx.get(token, C.UNKNOWN_IDX) for token in tokens]
    ...
    def __setitem__(self, tokens, new_vectors):
        ...
        assert new_vectors.shape == (len(tokens), self._idx_to_vec.shape[1]), \
            ...
            'new_vectors must be equal to the dimension of embedding of the glossary.'
        ...
        for token in tokens:
            if token in self._token_to_idx:
                ...
                raise ValueError('Token %s is unknown. To update the embedding vector for an '
                                 ...
                                 'updates.' % (token, self._idx_to_token[C.UNKNOWN_IDX]))
    ...
    def _check_pretrained_file_names(cls, file_name):
        ...
        if file_name not in cls.pretrained_file_name_sha1:
            raise KeyError('Cannot find pre-trained file %s for token embedding %s. Valid '
...
class GloVe(TokenEmbedding):
    ...
    Parameters
    ----------
    file_name : str, default 'glove.6B.50d.txt'
    ...
    def __init__(self, file_name='glove.6B.50d.txt',
                 embedding_root=os.path.join('~', '.mxnet', 'embedding'),
                 init_unknown_vec=nd.zeros, **kwargs):
        GloVe._check_pretrained_file_names(file_name)

        super(GloVe, self).__init__(**kwargs)
        pretrained_file_path = GloVe._get_pretrained_file(embedding_root, file_name)
...
class FastText(TokenEmbedding):
    ...
    def __init__(self, file_name='wiki.simple.vec',
                 embedding_root=os.path.join('~', '.mxnet', 'embedding'),
                 init_unknown_vec=nd.zeros, **kwargs):
        FastText._check_pretrained_file_names(file_name)

        super(FastText, self).__init__(**kwargs)
        pretrained_file_path = FastText._get_pretrained_file(embedding_root, file_name)
