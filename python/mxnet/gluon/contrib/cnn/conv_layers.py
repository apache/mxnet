...
class ModulatedDeformableConvolution(HybridBlock):
    ...
    def __init__(self, channels, kernel_size=(1, 1), strides=(1, 1), padding=(0, 0), dilation=(1, 1), groups=1,
                 num_deformable_group=1, layout='NCHW', in_channels=0, activation=None, modulated=True,
                 weight_initializer=None, bias_initializer='zeros', use_bias=True, 
                 offset_weight_initializer='zeros', offset_bias_initializer='zeros', offset_use_bias=True,
                 op_name='DeformableConvolution', adj=None, prefix=None, params=None):
        ...
        with self.name_scope():
            ...
            if self._modulated:
                self.offset_channels = 3 * kernel_size[0] * kernel_size[1] * num_deformable_group
                self.mask_channel_begin_index = 2 * kernel_size[0] * kernel_size[1] * num_deformable_group 
            else:
                self.offset_channels = 2 * kernel_size[0] * kernel_size[1] * num_deformable_group


            self._kwargs_offset = {
                'kernel': kernel_size, 'stride': strides, 'dilate': dilation,
                'pad': padding, 'num_filter': self.offset_channels, 'num_group': groups,
                'no_bias': not offset_use_bias, 'layout': layout}
    ...
    def hybrid_forward(self, F, x, offset_weight, weight, offset_bias=None, bias=None):

        if offset_bias is None:
            offset = F.Convolution(x, offset_weight, cudnn_off=False, **self._kwargs_offset)
        else:
            offset = F.Convolution(x, offset_weight, offset_bias, cudnn_off=False, **self._kwargs_offset)
            
        if self._modulated:
            offset_only = F.slice_axis(offset, axis=1, begin=0, end=self.mask_channel_begin_index)
            mask        = F.slice_axis(offset, axis=1, begin=self.mask_channel_begin_index, end=None)
            mask = F.sigmoid(mask)

            if bias is None:
                act = F.contrib.ModulatedDeformableConvolution(data=x, offset=offset_only, mask=mask, 
                                                    weight=weight, **self._kwargs_deformable_conv)
            else:
                act = F.contrib.ModulatedDeformableConvolution(data=x, offset=offset_only, mask=mask,
                                                    weight=weight, bias=bias, **self._kwargs_deformable_conv)
        else:
            if bias is None:
                act = F.contrib.ModulatedDeformableConvolution(data=x, offset=offset, weight=weight,
                                                    **self._kwargs_deformable_conv)
            else:
                act = F.contrib.ModulatedDeformableConvolution(data=x, offset=offset, weight=weight,
                                                    bias=bias, **self._kwargs_deformable_conv)
    ...
    def __repr__(self):
        s = 'modulated, ' if self._modulated else ''
        s += '{name}({mapping}, kernel_size={kernel}, stride={stride}'
        len_kernel_size = len(self._kwargs_deformable_conv['kernel'])
        if self._kwargs_deformable_conv['pad'] != (0,) * len_kernel_size:
            s += ', padding={pad}'
