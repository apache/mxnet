...
```bash
sudo apt-get update
sudo apt-get --no-install-recommends install -y build-essential git ninja-build ccache libopenblas-dev libopencv-dev cmake
```
...
not available on your system, you can install it via `sudo apt-get --no-install-recommends install -y
...
```bash
sudo apt-get --no-install-recommends install -y libmouse-perl pdl cpanminus swig libgraphviz-perl
...
```bash
sudo apt-get update
sudo apt-get --no-install-recommends install -y r-base-core r-cran-devtools libcairo2-dev libxml2-dev
```
