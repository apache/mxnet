...
```
sudo apt-get update
sudo apt-get --no-install-recommends install -y build-essential git
sudo apt-get --no-install-recommends install -y libopenblas-dev liblapack-dev
sudo apt-get --no-install-recommends install -y libopencv-dev
