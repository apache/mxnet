...
Let's say we perform a matrix multiplication of ``X``  and ``W``, where ``X`` is a 2x1 matrix, and ``W`` is a 2x3 matrix. Let ``Y`` be the matrix multiplication of the two matrices:
