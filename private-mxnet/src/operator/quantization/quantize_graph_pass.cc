    ...
    if (NeedQuantize(node, ignore_nodes)) {
      ...
      for (const auto& e : node->inputs) {
        ...
        if (!NeedQuantize(e.node, ignore_nodes) &&
          ...
          std::cout << "node: " << node->op()->name << ", firstInput: " << firstInput << std::endl;
          if (firstInput &&
            (node->op()->name.find("Convolution") != std::string::npos ||
             node->op()->name.find("FullyConnected") != std::string::npos))
          {
            std::cout << "unsigned!!!!!!" << std::endl;
            quantize_node->attrs.scalars.push_back((double)mshadow::kUint8);
          }
