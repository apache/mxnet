...
shutil.rmtree(os.path.join(CURRENT_DIR, 'include'), ignore_errors=True)
...
shutil.copytree(os.path.join(CURRENT_DIR, 'mxnet-build/include'),
...
package_data = {'mxnet': [os.path.join('mxnet', os.path.basename(LIB_PATH[0]))],
                'dmlc_tracker': [],
                'include': []}
for root, dirs, files in os.walk(os.path.join(CURRENT_DIR, 'include')):
    for file in files:
        package_data['mxnet'].append(os.path.abspath(os.path.join(root, file)))
...
if variant.endswith('MKL'):
    if platform.system() == 'Darwin':
        ...
        pass
        # shutil.copy(os.path.join(os.path.dirname(LIB_PATH[0]), 'mkldnn.dll'), os.path.join(CURRENT_DIR, 'mxnet'))
