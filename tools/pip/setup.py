...
if variant.endswith('MKL'):
    if platform.system() == 'Darwin':
        shutil.copy(os.path.join(os.path.dirname(LIB_PATH[0]), 'libdnnl.1.dylib'), os.path.join(CURRENT_DIR, 'mxnet'))
        package_data['mxnet'].append('mxnet/libdnnl.1.dylib')
    else:
        shutil.copy(os.path.join(os.path.dirname(LIB_PATH[0]), 'libdnnl.so.1'), os.path.join(CURRENT_DIR, 'mxnet'))
