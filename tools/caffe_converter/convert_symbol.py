...
def main():
	...
	proto = readProtoSolverFile(args.caffe_proto_file)
	connection = dict()
	symbols = dict()
	top = dict()
	mapping = {'data' : 'data'}
	...
	for i in range(len(layer)):
		...
		if layer[i].type == 'Concat':
			...
			need_flatten[name] = True
		if type_string == '':
			print 'Unknown Layer %s!' % layer[i].type 
			return
