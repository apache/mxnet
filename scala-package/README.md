...
**Note:** ```<version>[1.10.0-SNAPSHOT,)</version>``` indicates that we will fetch packages with version 1.10.0 or higher. This will always ensure that the pom.xml is able to fetch the latest and greatest jar files from Maven Snapshot repository.
