    ...
    for order in [1, 2]:
        for dtype in [np.float16, np.float32, np.float64]:
            for i in range(in_data_dim):
                for out_dtype in ['float16', 'float32', 'float64']:
                    ...
                    check_symbolic_forward(norm_sym, [in_data.astype(dtype)], [npy_out.astype(out_dtype)],
                                           rtol=1e-3 if out_dtype == 'float16' else 1e-3,
                                           atol=1e-5 if out_dtype == 'float16' else 1e-5, ctx=ctx)
                    # check_symbolic_backward(norm_sym, [in_data.astype(dtype)], [np.ones(npy_out.shape).astype(out_dtype)],
                    #                         [npy_out_backward],
                    #                         rtol=1e-4 if dtype is np.float16 else 1e-5,
