...
def test_unary_math_operators():
    ...
    if have_scipy:
        ...
        unary_ops['gammaln'] = [lambda x: mx.sym.gammaln(x),
                                ...
                                0.01, 20.0]
        unary_ops['logcdf_normal'] = [lambda x: mx.sym.logcdf_normal(x),
                                      lambda x: scipy_stats.norm.logcdf(x),
                                      lambda x: np_derivlogcdf_normal(x),
                                      -10.0, 5.0]
        unary_ops['derivlogcdf_normal'] = [lambda x: mx.sym.derivlogcdf_normal(x),
                                           lambda x: np_derivlogcdf_normal(x),
    ...
    for name, op in unary_ops.items():
        ...
        if name == 'gammaln' or name == 'gamma' or name == 'logcdf_normal' \
                or name == 'derivlogcdf_normal':
            rtol = rtol_np_less
            atol = atol_np_less
        else:
            rtol = rtol_np
