...
def test_np_bitwise_and():
    ...
    # Test broadcast
    # values = [(1, _np.array([1, 0, -1])),
    #           (_np.array([1, 0, -1]), 0),
    #           (3, 17),
    #           ]
    values = [
              (np.array([1, 0, -1]).astype(_np.dtype(int)), 0), # (mxnet.numpy.ndarray, int)
              ]
    ...
    for hybridize in [True, False]:
        for value in values:
            ...
            print(x2)
            print(type(x2))
            # input is of type 'numpy.ndarray'
            np_out = _np.bitwise_and(x1.asnumpy(), x2)
            with mx.autograd.record():
                # input for test_bitwise_and is of class 'mxnet.numpy.ndarray'
                mx_out = test_bitwise_and(x1, x2) # symbolic
            assert mx_out.shape == np_out.shape
            assert_almost_equal(mx_out.asnumpy(), np_out, rtol=1e-3, atol=1e-5)
