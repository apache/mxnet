...
def test_np_bitwise_xor():
    ...
    for hybridize in [True, False]:
        for shape in shapes:
            ...
            # np_backward = ...
            # x1_np_backward = rand_ndarray(shape[0]).as_np_ndarray()
            # x2_np_backward = rand_ndarray(shape[1]).as_np_ndarray()
            x1_np_backward = _np.zeros(shape[0])
            x2_np_backward = _np.zeros(shape[1])
            assert_almost_equal(x_1.grad.asnumpy(), x1_np_backward, atol=1e-3, rtol=1e-5)
            assert_almost_equal(x_2.grad.asnumpy(), x2_np_backward, atol=1e-3, rtol=1e-5)
