...
def test_np_logaddexp():
    ...
    for shape_a, shape_b in shapes:
        for hybridize in [True, False]:
            ...
            np_backward_lhs = _np.exp(lhs.asnumpy()) / (_np.exp(lhs.asnumpy()) + _np.exp(rhs.asnumpy()))
            np_backward_rhs = _np.exp(rhs.asnumpy()) / (_np.exp(lhs.asnumpy()) + _np.exp(rhs.asnumpy()))
            with mx.autograd.record():
            ...
            for n_dim in range(len(shape_a)):
                if (shape_a[n_dim] != shape_b[n_dim]):
                    if (shape_a[n_dim] > shape_b[n_dim]):
                        np_backward_rhs = np_backward_rhs.sum(axis=n_dim, keepdims=True)
                    else:
                        np_backward_lhs = np_backward_lhs.sum(axis=n_dim, keepdims=True)
            assert_almost_equal(lhs.grad.asnumpy(), np_backward_lhs, rtol=1e-3, atol=1e-5)
            assert_almost_equal(rhs.grad.asnumpy(), np_backward_rhs, rtol=1e-3, atol=1e-5)
            # Test imperative once again
