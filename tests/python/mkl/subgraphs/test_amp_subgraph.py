...
def same_graph_structure(symnet1: mx.sym.Symbol, symnet2):
...
def check_amp_fuse(sym_fp32: mx.symbol.Symbol, args, dtype, expected_sym=None, quantized_nodes=[],
                   rtol=0.05, inputs_casted=True):
  sym_lp, args_lp, _ = amp.convert_model(sym_fp32, args, {}, target_dtype=dtype,
                                         excluded_sym_names=quantized_nodes,
                                         cast_optional_params=True)
  ex_lp = sym_lp._bind(mx.cpu(), args_lp)

  sym_sg_fp32 = sym_fp32.get_backend_symbol(SG_PASS_NAME)  # amp pass works only on mkldnn nodes
  ...
  out_lp = ex_lp.forward()[0]
  out_sg_lp = ex_sg_lp.forward()[0]

  # check outputs
  assert out_lp.dtype == out_sg_lp.dtype
  assert_almost_equal(out_lp, out_sg_lp, rtol=rtol)

  # check graph
  if expected_sym is not None:
    assert same_graph_structure(sym_sg_lp, expected_sym)
    if inputs_casted:
      # if inputs are casted to low precision, they should not be casted when they are not
      # provieded in amp.convert_model call
      assert not same_graph_structure(sym_sg_lp_no_args, expected_sym)
...
def test_amp_fc():
  ...
  weight1_nd = mx.random.normal(-1, 1, shape=(16, data_nd.shape[1]), dtype='float32')
  weight2_nd = mx.random.normal(-1, 1, shape=(8, weight1_nd.shape[0]), dtype='float32')
  bias1_nd = mx.random.normal(-1, 1, shape=(weight1_nd.shape[0],), dtype='float32')
  ...
  check_amp_fuse(sym, args, 'bfloat16', exp_sym, ['sg_mkldnn_fully_connected_1'])


def test_amp_conv():
  data_nd = mx.random.uniform(-1, 1, shape=(1, 3, 8, 8), dtype='float32')
  weight1_nd = mx.random.normal(-1, 1, shape=(8, 3, 3, 3), dtype='float32')
  weight2_nd = mx.random.normal(-1, 1, shape=(4, 8, 3, 3), dtype='float32')
  bias1_nd = mx.random.normal(-1, 1, shape=(weight1_nd.shape[0],), dtype='float32')
  data = mx.symbol.Variable('data')
  weight1 = mx.symbol.Variable('weight1')
  weight2 = mx.symbol.Variable('weight2')
  ...
  sym = mx.symbol.Convolution(data=conv1, weight=weight2, no_bias=True, kernel=weight1_nd.shape[2:],
  ...
  exp_conv1 = mx.symbol.Convolution(data=data, weight=weight1, bias=bias1, kernel=weight1_nd.shape[2:],
                                    num_filter=weight1_nd.shape[0])
  exp_sym = mx.symbol.Convolution(data=exp_conv1, weight=weight2, no_bias=True, kernel=weight1_nd.shape[2:],
                                  num_filter=weight2_nd.shape[0])
  exp_sym = exp_sym.get_backend_symbol(SG_PASS_NAME)

  check_amp_fuse(sym, args, 'bfloat16', exp_sym, ['sg_mkldnn_conv_1'])
...
# def test_amp_qk():
#   heads = 12
#   data_nd = mx.random.uniform(-1, 1, shape=(32, 1, 3*8*heads), dtype='float32')
#   data = mx.symbol.Variable('data')
#   qk1 = mx.symbol.contrib.interleaved_matmul_selfatt_qk(data, heads)
#   sym = mx.symbol.contrib.interleaved_matmul_selfatt_qk(qk1, heads)
#   args = {'data': data_nd}

#   exp_qk1 = mx.symbol.contrib.interleaved_matmul_selfatt_qk(data, heads)
#   exp_sym = mx.symbol.contrib.interleaved_matmul_selfatt_qk(exp_qk1, heads)
#   exp_sym = exp_sym.get_backend_symbol(SG_PASS_NAME)
...
def test_amp_common_params():
  data_nd = mx.random.uniform(-1, 1, shape=(1, 8), dtype='float32')
  weight_nd = mx.random.normal(-1, 1, shape=(16, data_nd.shape[1]), dtype='float32')
  bias_nd = mx.random.normal(-1, 1, shape=(weight_nd.shape[0],), dtype='float32')
  data = mx.symbol.Variable('data')
  ...
  exp_fc3 = mx.symbol.FullyConnected(data=data, weight=weight, bias=bias,
                                     num_hidden=weight_nd.shape[0])
  exp_sym = mx.symbol.Concat(exp_fc1, exp_fc2, exp_fc3)
  exp_sym = exp_sym.get_backend_symbol(SG_PASS_NAME)

  check_amp_fuse(sym, args, 'bfloat16', exp_sym, ['sg_mkldnn_fully_connected_2'],
                 inputs_casted=False)
