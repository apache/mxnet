...
```
#install maven
sudo apt-get --no-install-recommends install -y ca-certificates apt-utils maven -y #>/dev/null

#install svn
sudo apt-get --no-install-recommends install -y ca-certificates apt-utils subversion -y #>/dev/null
