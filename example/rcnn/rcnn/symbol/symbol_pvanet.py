...
def get_pvanet_rcnn(num_classes=config.NUM_CLASSES):
    """
    Fast R-CNN with pvanet conv layers
...
def get_pvanet_rcnn_test(num_classes=config.NUM_CLASSES):
    """
    Fast R-CNN Network with pvanet
    :param num_classes: used to determine output size
    :return: Symbol
    """
    data = mx.symbol.Variable(name="data")
...
def get_pvanet_rpn(num_anchors=config.NUM_ANCHORS):
    """
    Region Proposal Network with pvanet
...
def get_pvanet_rpn_test(num_anchors=config.NUM_ANCHORS):
    """
    Region Proposal Network with pvanet
    :param num_anchors: used to determine output size
    :return: Symbol
...
def get_pvanet_train(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    # Fast R-CNN
    pool5 = mx.symbol.ROIPooling(
        name='roi_pool5', data=relu5_3, rois=rois, pooled_size=(6, 6), spatial_scale=1.0 / config.RCNN_FEAT_STRIDE)
    # group 6
    flatten = mx.symbol.Flatten(data=pool5, name="flatten")
    fc6 = mx.symbol.FullyConnected(data=flatten, num_hidden=4096, name="fc6")
    relu6 = mx.symbol.Activation(data=fc6, act_type="relu", name="relu6")
