...
def get_zf_conv(data):
    ...
    conv1 = mx.symbol.Convolution(
        data=data, kernel=(7, 7), pad=(0, 0), stride=(2, 2), num_filter=96, name="conv1")
    relu1 = mx.symbol.Activation(data=conv1, act_type="relu", name="relu1")
    # conv1_2 = mx.symbol.Convolution(
    #     data=relu1_1, kernel=(3, 3), pad=(1, 1), num_filter=64, name="conv1_2")
    # relu1_2 = mx.symbol.Activation(data=conv1_2, act_type="relu", name="relu1_2")
    pool1 = mx.symbol.Pooling(
        data=relu1, pool_type="max", kernel=(2, 2), stride=(2, 2), name="pool1")
    # group 2
    conv2 = mx.symbol.Convolution(
        data=pool1, kernel=(5, 5), pad=(0, 0), stride=(2, 2), num_filter=256, name="conv2")
    ...
    pool2 = mx.symbol.Pooling(
        data=relu2, pool_type="max", kernel=(2, 2), stride=(2, 2), name="pool2")
    # group 3
    conv3 = mx.symbol.Convolution(
        data=pool2, kernel=(3, 3), pad=(1, 1), num_filter=384, name="conv3")
    ...
    #     data=relu3, pool_type="max", kernel=(2, 2), stride=(2, 2), name="pool3")
    # group 4
    conv4 = mx.symbol.Convolution(
        data=relu3, kernel=(3, 3), pad=(1, 1), num_filter=384, name="conv4")
    ...
    conv5 = mx.symbol.Convolution(
        data=relu4, kernel=(3, 3), pad=(1, 1), num_filter=256, name="conv5")
    ...
    pool5 = mx.symbol.Pooling(
        data=relu5, pool_type="max", kernel=(2, 2), stride=(2, 2), name="pool5")

    return pool5


def get_zf_rcnn(num_classes=21):
    """
...
def get_zf_rcnn_test(num_classes=21):
    """
...
def get_zf_rpn(num_classes=21, num_anchors=9):
    """
...
def get_zf_rpn_test(num_classes=21, num_anchors=9):
    """
    Region Proposal Network with ZF
    :param num_classes: used to determine output size
    :param num_anchors: used to determine output size
    :return: Symbol
    """
...
def get_zf_test(num_classes=21, num_anchors=9):
    """
    Faster R-CNN test with ZF 16 conv layers
    :param num_classes: used to determine output size
    :param num_anchors: used to determine output size
    :return: Symbol
    """
    ...
    # shared convolutional layers
    relu5 = get_zf_conv(data)

    # RPN
    rpn_conv = mx.symbol.Convolution(
        data=relu5, kernel=(3, 3), pad=(1, 1), num_filter=512, name="rpn_conv_3x3")
    rpn_relu = mx.symbol.Activation(data=rpn_conv, act_type="relu", name="rpn_relu")
    rpn_cls_score = mx.symbol.Convolution(
        data=rpn_relu, kernel=(1, 1), pad=(0, 0), num_filter=2 * num_anchors, name="rpn_cls_score")
    rpn_bbox_pred = mx.symbol.Convolution(
    ...
    # Fast R-CNN
    # pool5 = mx.symbol.ROIPooling(
    #     name='roi_pool5', data=relu5, rois=rois, pooled_size=(7, 7), spatial_scale=0.0625)
    # group 6
    flatten = mx.symbol.Flatten(data=relu5, name="flatten")
    fc6 = mx.symbol.FullyConnected(data=flatten, num_hidden=4096, name="fc6")
    relu6 = mx.symbol.Activation(data=fc6, act_type="relu", name="relu6")
    drop6 = mx.symbol.Dropout(data=relu6, p=0.5, name="drop6")
    # group 7
...
def get_faster_rcnn(num_classes=21, num_anchors=9):
    """
    Faster R-CNN with ZF 16 conv layers
    :param num_classes: used to determine output size
    :return: Symbol
    """
    data = mx.symbol.Variable(name="data")
