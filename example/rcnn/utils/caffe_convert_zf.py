...
import numpy as np
fast_rcnn_path = '../' #None
sys.path.insert(0, fast_rcnn_path)
# sys.path.insert(0, os.path.join(fast_rcnn_path, 'python'))
# sys.path.insert(0, os.path.join(fast_rcnn_path, 'lib'))
import caffe
from rcnn.symbol_zf import get_zf_test
...
def load_model(caffeproto, caffemodel, arg_shape_dic):
    ...
    for layer_name, layer_type, layer_blobs in iter:
        print 'processing: ',layer_name, layer_type, layer_blobs
        if layer_type == 'Convolution' or layer_type == 'InnerProduct' or layer_type == 4 or layer_type == 14:
            ...
            if weight_name not in arg_shape_dic:
                print weight_name + ' not found in arg_shape_dic:',arg_shape_dic.keys()
                continue
            print wmat.shape,arg_shape_dic[weight_name]
...
proto_path = os.path.join(fast_rcnn_path, 'models', 'ZF', 'test.prototxt')
model_path = os.path.join(fast_rcnn_path, 'models', 'ZF', 'ZF.caffemodel')
