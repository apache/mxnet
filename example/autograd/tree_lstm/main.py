...
import argparse, cPickle, math, os, random, time
import logging
...
def test(ctx, data_iter, best, mode='validation', num_iter=-1):
    ...
    if mode == 'validation' and num_iter >= 0:
        if test_r >= best:
            ...
            net.all_params().save('childsum_tree_lstm_{}.params'.format(num_iter))
...
def train(epoch, ctx, train_data, dev_data):
    if isinstance(ctx, mx.Context):
        ctx = [ctx]
    net.all_params().initialize(mx.init.Xavier(magnitude=2.24), ctx=ctx[0])
    net.embed.weight.set_data(vocab.embed.as_in_context(ctx[0]))
    trainer = foo.Trainer(net.all_params(), optimizer, {'learning_rate': opt.lr, 'wd': opt.wd})
    ...
    for i in range(epoch):
        ...
        for j in tqdm(range(batches), desc='Training epoch {}'.format(i)):
            l_tree, l_sent, r_tree, r_sent, label = train_data.next()
            with ag.record():
                z = net(mx.nd, l_sent, r_sent, l_tree, r_tree)
                loss = foo.loss.kl_divergence_loss(z, to_target(label).as_in_context(ctx[0]))
                loss.backward()
                preds.append(z)
            if (j+1) % batch_size == 0:
