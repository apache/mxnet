...
from mxnet.contrib.nn.legacy import *

import sys
sys.path.append('..')
from data import *

partial_shape = lambda filters: ((0, filters, 0, 0),)

def conv3x3(filters, stride, in_filters):
    stride = (stride, stride)
    return Convolution(num_filter=filters, kernel=(3, 3), stride=stride, pad=(1, 1),
                       no_bias=True, shape=partial_shape(in_filters))
    '''
...
def conv1x1(filters, stride, in_filters):
    stride = (stride, stride)
    return Convolution(num_filter=filters, kernel=(1, 1), stride=stride, pad=(0, 0),
                       no_bias=True, shape=partial_shape(in_filters))
...
class BasicBlock(nn.Layer):
    def __init__(self, filters, stride, downsample=False, in_filters=0, **kwargs):
        ...
        with self.scope:
            self.bn1 = BatchNorm(shape=partial_shape(in_filters))
            self.conv1 = conv3x3(filters, stride, in_filters)
            self.bn2 = BatchNorm(shape=partial_shape(filters))
            self.conv2 = conv3x3(filters, 1, filters)
            if downsample:
                self.downsample = conv1x1(filters, stride, in_filters)
                '''
...
class Bottleneck(nn.Layer):
    def __init__(self, filters, stride, downsample=False, in_filters=0, **kwargs):
        ...
        with self.scope:
            ...
            self.bn2 = BatchNorm(shape=partial_shape(filters//4))
            self.conv2 = conv3x3(filters//4, stride, filters//4)
            self.bn3 = BatchNorm(shape=partial_shape(filters//4))
...
class Resnet(nn.Layer):
    def __init__(self, block, classes, layers, filters, thumbnail=False, **kwargs):
        ...
        with self.scope:
            ...
            self.bn_data = BatchNorm(shape=partial_shape(3), fix_gamma=True)
            # self.bn_data = BatchNorm(num_features=3, scale=False, center=False)
            if thumbnail:
                self.conv0 = conv3x3(filters[0], 1, 3)
            else:
                self.conv0 = Convolution(num_filter=filters[0],
                                         kernel=(7, 7), stride=(2, 2), pad=(3, 3),
                                         use_bias=False, shape=partial_shape(3))
                '''
                self.conv0 = nn.Conv2D(filters[0], 7, 2, 3, use_bias=False,
                                       in_filters=3)
                '''
                self.bn0 = BatchNorm(shape=partial_shape(filters[0]))
                self.pool0 = Pooling(pool_type='max',
                                     kernel=(3, 3), stride=(2, 2), pad=(1, 1),
                                     shape=partial_shape(filters[0]))
            ...
            self.bn1 = BatchNorm(shape=partial_shape(in_filters))
            self.pool1 = Pooling(pool_type='avg', kernel=(1, 1), global_pool=True,
                                 shape=partial_shape(filters[0]))
            # self.pool1 = nn.GlobalAvgPool2D()
            self.dense1 = FullyConnected(num_hidden=classes,
                                         shape=((0, in_filters),))
...
if __name__ == '__main__':
    train(200, [mx.gpu(i) for i in range(1)])
    '''
    import logging
