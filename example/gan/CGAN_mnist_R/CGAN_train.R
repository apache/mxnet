...
arg_param_ini_G <- mx.init.create(initializer = initializer,
                                  shape.array = mx.symbol.infer.shape(G_sym,
                                                                      data=data_shape_G)$arg.shapes,
                                  ctx = devices)
aux_param_ini_G <- mx.init.create(initializer = initializer,
                                  shape.array = mx.symbol.infer.shape(G_sym,
                                                                      data=data_shape_G)$aux.shapes,
                                  ctx = devices)

arg_param_ini_D <- mx.init.create(initializer = initializer,
                                  shape.array = mx.symbol.infer.shape(D_sym, data=data_shape_D,
                                                                      digit=digit_shape_D)$arg.shapes,
                                  ctx = devices)

aux_param_ini_D <- mx.init.create(initializer = initializer,
                                  shape.array = mx.symbol.infer.shape(D_sym, data=data_shape_D,
                                                                      digit=digit_shape_D)$aux.shapes,
                                  ctx = devices)
...
for (iteration in 1:2400) {
  ...
  mx.exec.update.arg.arrays(exec_D, arg.arrays = list(data=D_data_fake, digit=D_digit_fake,
                                                      label=mx.nd.array(rep(0, batch_size))),
                            match.name=TRUE)
  ...
  mx.exec.update.arg.arrays(exec_D, arg.arrays = list(data=D_data_real, digit=D_digit_real,
                                                      label=mx.nd.array(rep(1, batch_size))),
                            match.name=TRUE)
  mx.exec.forward(exec_D, is.train=T)
  mx.exec.backward(exec_D)
  update_args_D <- updater_D(weight = exec_D$ref.arg.arrays, grad = exec_D$ref.grad.arrays)
  ...
  exec_D_back <- mxnet:::mx.symbol.bind(symbol = D_sym, arg.arrays = exec_D$arg.arrays,
                                       ...
                                       ctx = devices)
  mx.exec.update.arg.arrays(exec_D_back, arg.arrays = list(data=D_data_fake, digit=D_digit_fake,
                                                           label=mx.nd.array(rep(1, batch_size))),
                            match.name=TRUE)
...
ifelse(!dir.exists(file.path(".", "models")), dir.create(file.path(".", "models")), "Folder already exists")
