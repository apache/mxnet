...
def InceptionFactoryA(data, num_1x1, num_3x3red, num_3x3, num_d3x3red, num_d3x3, pool, proj, name):
    ...
    # pool + proj
    pooling = mx.symbol.Pooling(data=data, kernel=(3, 3), stride=(1, 1), pad=(1, 1), pool_type=pool,pooling_convention='full', name=('%s_pool_%s_pool' % (pool, name)))
...
def DownsampleFactory(data, ch_3x3, name, attr):
    ...
    # pool
    pool = mx.symbol.Pooling(data=data, name=name+'_pool',kernel=(3, 3), stride=(2, 2), pad=(1, 1), pooling_convention='full',pool_type='max', attr=attr)
    # concat
...
def get_symbol(num_classes, image_shape, **kwargs):
    ...
    if height <= 28:
        ...
        pool = mx.symbol.Pooling(data=in5b, pool_type="avg", kernel=(7,7), name="global_pool", pooling_convention='full',attr=attr)
    else:
        # stage 1
        conv1 = ConvFactory(data=data, num_filter=64, kernel=(7, 7), stride=(2, 2), pad=(3, 3), name='1')
        pool1 = mx.symbol.Pooling(data=conv1, kernel=(3, 3), stride=(2, 2), name='pool_1', pooling_convention='full', pool_type='max')
        # stage 2
        ...
        pool2 = mx.symbol.Pooling(data=conv2, kernel=(3, 3), stride=(2, 2), pooling_convention='full', name='pool_2', pool_type='max')
        # stage 2
        ...
        # global avg pooling
        pool = mx.symbol.Pooling(data=in5b, kernel=(7, 7), stride=(1, 1), pooling_convention='full',name="global_pool", pool_type='avg')
