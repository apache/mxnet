...
get_data <- function(data_dir) {
  ...
  if ((!file.exists(paste0(data_dir,"train.rec"))) ||
      (!file.exists(paste0(data_dir,"test.rec" ))) ||
      (!file.exists(paste0(data_dir,"train.lst"))) ||
      (!file.exists(paste0(data_dir,"test.lst" )))    ) {
    setwd(data_dir)
    download.file(url='http://data.mxnet.io/mxnet/data/cifar10.zip',
                  destfile='cifar10.zip', method='wget')
    unzip("cifar10.zip", junkpaths = T)  # unzip only files not dirs
