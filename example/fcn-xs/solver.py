...
class Solver(object):
    ...
    def fit(self,
        ...
        mx.model._check_arguments(self.symbol)
        ...
        img_path_name = train_data.img_path
        ...
        for epoch in range(self.begin_epoch, self.num_epoch):
            ...
            for data in train_data:
                ...
                self.arg_params[label_name] = mx.nd.array(data[label_name].reshape(label_shape[0], label_shape[1]*label_shape[2]), self.ctx)
                ...
                if self.monitor is not None:
                    self.monitor.install(self.exector)
                    self.monitor.tic()
                ...
                for key, arr in update_dict.items():
                    if key != "deconv8_weight" and key != "bigscore_weight":
                ...
                pred = mx.nd.array(output_buff["softmax_output"].asnumpy().reshape(pred_shape[0], pred_shape[1], pred_shape[2]*pred_shape[3]))
            ...
            if epoch_end_callback != None and epoch % self.snapshot == 0:
