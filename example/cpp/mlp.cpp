...
#if MSHADOW_USE_CUDA
#define DEV_CTX (mxnet::Context::Create(mxnet::Context::kGPU, 0))// use no. 0 gpu
#else
...
class MLP{
	public:
		...
		void Train(){
			...
			arg_grad_store.push_back(mxnet::NDArray());// we don't need the grad of the input
			...
			arg_grad_store.push_back(mxnet::NDArray()); // neither do we need the grad of the loss
